...
internal sealed record WhisperCppCliCapabilities(
    ...
    string? MaximumSegmentLengthOption,
    ...
    public AudioTranscriptionProviderCapabilities ToPublic() =>
        new(
            ...
            supportsWordTimestamps:
                ...
                SplitOnWordOption is not null &&
                MaximumSegmentLengthOption is not null,
            supportsTemperature:
...
{
    private const int WordTimingMaximumSegmentLengthCharacters = 80;
    internal const double VadSamplesOverlapSeconds = 0.10;
        ...
        string? vadModelSha256 = null)
    {
        ...
        if (vadModelPath is not null)
        {
            ...
            arguments.Add(
                VadSamplesOverlapSeconds.ToString(
                    "0.00",
            ...
            normalized["vadSamplesOverlapSeconds"] =
                VadSamplesOverlapSeconds.ToString(
                    "0.00",
                    CultureInfo.InvariantCulture);
            if (options.RequestWordTimestamps)
            {
                normalized["vadWordTimestampClock"] =
                    "provider-time-map";
            }
        }
        ...
        if (options.RequestWordTimestamps)
        {
            string splitOnWord = capabilities.SplitOnWordOption ??
                throw new WhisperCppInitializationException(
                    "The installed whisper.cpp CLI does not advertise word-aligned segment splitting (--split-on-word).");
            string maximumSegmentLength =
                capabilities.MaximumSegmentLengthOption ??
                throw new WhisperCppInitializationException(
                    "The installed whisper.cpp CLI does not advertise the bounded segment length required for token timing (--max-len).");
            arguments.Add(splitOnWord);
            arguments.Add(maximumSegmentLength);
            arguments.Add(
                WordTimingMaximumSegmentLengthCharacters.ToString(
                    CultureInfo.InvariantCulture));
            normalized["wordTimestampSource"] =
                "full-json-token-timing";
            normalized["wordTimingMaximumSegmentLengthCharacters"] =
                WordTimingMaximumSegmentLengthCharacters.ToString(
                    CultureInfo.InvariantCulture);
        }
        ...
        if (capabilities.NoPrintsOption is not null &&
            !(options.RequestWordTimestamps && vadModelPath is not null))
        {
