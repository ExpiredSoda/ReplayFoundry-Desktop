...
internal sealed partial class YouTubeDataApiClient : IYouTubeDataApiClient
{
    private const int UploadChunkSize = 8 * 1024 * 1024;
    private const int MaximumChunkAttempts = 4;
    private const int MaximumFinalizationStatusAttempts = 3;
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (IsResumeIncomplete(response))
                    {
                        ...
                        {
                            UploadStatus status =
                                await QueryRecoverableUploadStatusAsync(
                                        uploadSession,
                                        accessToken,
                                        file.Length,
                                        cancellationToken)
                        ...
                        attempt < MaximumChunkAttempts)
                    {
                        await DelayBeforeRetryAsync(
                                ...
                                cancellationToken)
                            .ConfigureAwait(false);
                        UploadStatus status = await QueryRecoverableUploadStatusAsync(
                                uploadSession,
                                accessToken,
                                file.Length,
                                cancellationToken)
