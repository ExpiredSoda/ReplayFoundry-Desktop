...
internal static class Qwen3VlGroundedMetadataRules
{
    internal static readonly HashSet<string> NonRetrospectiveActionForms = new(
        [
            "am", "are", "is", "appear", "appears", "attack", "attacks", "beat", "beats",
            ...
            "center", "centers", "continue", "continues", "display",
    ...
    private static readonly Regex FirstPersonPossessive = new(
        ...
        RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
    private static readonly Regex FirstPersonNonSubject = new(
        @"\b(?:me|my|mine|us|our|ours)\b",
        RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        ...
        bool allowAutomaticCommentaryAttribution = false)
    {
        ...
        string historicalHashtagSuffix = $" {hashtag}";
        if (title.Contains('#') &&
            (!title.EndsWith(
                    historicalHashtagSuffix,
                    StringComparison.Ordinal) ||
                title[..^historicalHashtagSuffix.Length].Contains('#')))
        {
            failures.Add(
                $"title contains an embedded hashtag instead of either remaining " +
                $"hashtag-free or using the historical exact suffix '{hashtag}'");
        }
        ...
        bool reviewedCommentarySupportsDescription =
            HasReviewedCommentaryWordingSupport(request, description);
        if (Qwen3VlGroundedMetadataLanguageRules
                .UsesNonRetrospectiveTitleOpening(
                    title,
                    hashtag,
                    allowCopularCommentary:
                        reviewedCommentarySupportsTitle))
        {
            failures.Add(
        ...
        if (Qwen3VlGroundedMetadataLanguageRules
                .UsesNonRetrospectiveDescription(
                    description,
                    allowCopularCommentary:
                        reviewedCommentarySupportsDescription))
        {
    ...
    private static bool HasReviewedCommentaryWordingSupport(
        ClipEditorialMetadataRequest request,
        string audienceCopy)
    {
    ...
    private static bool HasReviewedFirstPersonCommentarySupport(
        ...
        string audienceCopy)
    {
        if (FirstPersonNonSubject.IsMatch(audienceCopy))
        {
            return false;
        }
        string[] directActions = FirstPersonSubjectAction.Matches(audienceCopy)
        ...
        string[] reviewedCreatorSpeech = request.Context.Transcripts
            .Where(static transcript =>
                transcript.Role.Role == AudioContentRole.CreatorSpeech &&
                transcript.MaySupportVerbatimAudienceCopy)
            .Select(static transcript => " " +
                Qwen3VlGroundedMetadataLanguageRules.NormalizeWords(
                    transcript.Text) + " ")
            .ToArray();
        return reviewedCreatorSpeech.Length > 0 && directActions.All(action =>
            reviewedCreatorSpeech.Any(transcript =>
                transcript.Contains(" i " + action + " ", StringComparison.Ordinal) ||
                transcript.Contains(" we " + action + " ", StringComparison.Ordinal) ||
                transcript.Contains(" i had " + action + " ", StringComparison.Ordinal) ||
                transcript.Contains(" we had " + action + " ", StringComparison.Ordinal)));
    }
