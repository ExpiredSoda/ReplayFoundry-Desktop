...
internal static class Qwen3VlGroundedMetadataRerollDiversityPolicy
{
    ...
    internal static bool IsValidReportedProvenance(
        int priorAcceptedTitleCount,
        Qwen3VlGroundedMetadataRerollTitleDiversityCode code,
        int numerator,
        int denominator)
    {
        if (priorAcceptedTitleCount is < 0 or
            > Qwen3VlGroundedMetadataGenerator.MaximumCases - 1 ||
        ...
        bool atOrAboveThreshold =
            (long)numerator * SimilarityThresholdDenominator >=
            (long)SimilarityThresholdNumerator * denominator;
        return code switch
        {
            Qwen3VlGroundedMetadataRerollTitleDiversityCode.NoComparablePrior =>
                priorAcceptedTitleCount == 0 && numerator == 0 && denominator == 1,
            Qwen3VlGroundedMetadataRerollTitleDiversityCode.ExactCanonicalTitle =>
                priorAcceptedTitleCount > 0 && numerator == 1 && denominator == 1,
            Qwen3VlGroundedMetadataRerollTitleDiversityCode.HighTokenOverlap =>
                priorAcceptedTitleCount > 0 && atOrAboveThreshold,
            Qwen3VlGroundedMetadataRerollTitleDiversityCode.MateriallyDistinct =>
                priorAcceptedTitleCount > 0 && !atOrAboveThreshold,
            _ => false,
        };
    ...
    internal static Qwen3VlGroundedMetadataGenerationValidation
        ...
        bool retainReviewableMismatch)
    {
        ArgumentNullException.ThrowIfNull(actual);
        ArgumentNullException.ThrowIfNull(reported);
        bool matches =
            reported.PriorAcceptedTitleCount == actual.ComparablePriorCount &&
        ...
        if (!retainReviewableMismatch)
        {
            throw new Qwen3VlOutputParseException(
                "Grounded Qwen reroll-diversity provenance is invalid.");
        }
        string[] issues = reported.MetadataReviewIssues
            .Append("RerollDiversityProvenanceRecomputed")
            .Distinct(StringComparer.Ordinal)
            .ToArray();
        return reported with
        {
            PriorAcceptedTitleCount = actual.ComparablePriorCount,
            RerollTitleDiversityCode = actual.Code,
            RerollTitleTokenJaccardNumerator =
                actual.TokenJaccardNumerator,
            RerollTitleTokenJaccardDenominator =
                actual.TokenJaccardDenominator,
            MetadataReviewRequired = true,
            MetadataReviewIssues = issues,
        };
    }
