...
internal sealed class Qwen3VlVerifiedModelLease : IDisposable
{
    ...
    internal bool Matches(VisualSemanticModelManifest model) =>
        model is not null &&
        _model.ManifestSha256.Equals(
            model.ManifestSha256,
            StringComparison.OrdinalIgnoreCase) &&
        Path.GetFullPath(_model.ModelDirectoryPath).Equals(
            Path.GetFullPath(model.ModelDirectoryPath),
            StringComparison.OrdinalIgnoreCase);
