...
internal static class Qwen3VlGroundedMetadataRecoverySelection
{
    internal static void Validate(
        ...
        bool recoveryCandidateSelectionModuleSupported,
        ...
        bool editorialResponsibilityModulesSupported)
    {
        ...
        int greedyCount = synthesisRecoveryPoolApplied
            ...
            : synthesisPassCount;
        int? selectedAttestationIndex = recoveredRejectedLanguage
            ? null
            : synthesisRecoveryPoolApplied &&
                synthesisRecoveryPoolSelectedCandidateOrdinal is int
                    selectedCandidateOrdinal
                ? greedyCount + selectedCandidateOrdinal - 1
        ...
        for (int index = 0; validAttestations && index < attestations.Count; index++)
        {
            ...
            bool accepted = !recoveredRejectedLanguage &&
                index == selectedAttestationIndex;
            if (!accepted && rejectionIndex >= rejectedValidationRules.Count)
            {
                validAttestations = false;
                continue;
            }
            string? expectedRejection = accepted
                ? null
                : rejectedValidationRules[rejectionIndex++];
        ...
        validAttestations = validAttestations &&
            rejectionIndex == rejectedValidationRules.Count;
        ...
        bool validFinalOutput = attestations.Count > 0 &&
            (recoveredRejectedLanguage
                ? editorialRephrase is not null &&
                    editorialRephrase.RawOutputSha256 is not null &&
                    editorialRephrase.RawOutputSha256.Equals(
                        decodedTextSha256,
                        StringComparison.OrdinalIgnoreCase)
                : selectedAttestationIndex is int selectedIndex &&
                    selectedIndex >= 0 &&
                    selectedIndex < attestations.Count &&
                    attestations[selectedIndex].OutputSha256.Equals(
                        decodedTextSha256,
                        StringComparison.OrdinalIgnoreCase));
        bool validLanguageRecovery = !recoveredRejectedLanguage ||
            ...
            editorialRephrase.SourceJsonSha256 is not null &&
            editorialRephrase.OutputJsonSha256 is not null &&
        ...
        bool validPoolSource = !synthesisRecoveryPoolApplied ||
            ...
            (recoveredRejectedLanguage
                ...
                : synthesisRecoveryPoolSelectedCandidateOrdinal is >= 1 &&
