...
internal static class Qwen3VlGroundedMetadataRecoveryParser
{
        ...
        Qwen3VlGroundedMetadataGenerationSchemaProfile profile)
    {
        ...
        if (profile.PacketReuse)
        {
            RequireText(
                ...
                profile.EditorialFraming
                    ? GroundingPacketSchemaVersion
                    : PreviousGroundingPacketSchemaVersion);
        ...
        out int? denominator)
    {
        ...
        if (!Qwen3VlGroundedMetadataRerollDiversityPolicy
                .IsValidReportedProvenance(
                    priorAcceptedTitleCount.Value,
                    parsed,
                    numerator.Value,
                    denominator.Value))
        {
            throw new Qwen3VlOutputParseException(
                "Grounded Qwen reported invalid reroll-diversity provenance.");
        }
    }
