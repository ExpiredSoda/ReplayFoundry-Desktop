...
internal static class Qwen3VlGroundedMetadataResultParser
{
        ...
        Dictionary<string, Qwen3VlGroundingPacketReceipt>? verifiedPacketReceipts = null)
    {
        ...
        if (outcomes.Any(static outcome => !outcome.IsAccepted))
        {
            throw new Qwen3VlOutputParseException(
                "Grounded Qwen metadata contains a reviewable per-case failure.");
        }
        return outcomes.Select(static outcome => outcome.Draft!).ToArray();
    }

    internal static IReadOnlyList<ClipEditorialMetadataBatchOutcome>
        ParseOutcomes(
            string json,
            IReadOnlyList<ClipEditorialMetadataRequest> requests,
            Qwen3VlQualifiedEditorialRuntime runtime,
            ...
            Dictionary<string, Qwen3VlGroundingPacketReceipt>? verifiedPacketReceipts = null)
    {
        ...
        var outcomes = new List<ClipEditorialMetadataBatchOutcome>(
            requests.Count);
        ...
        foreach (JsonElement result in results.EnumerateArray())
        {
            ...
            if (result.TryGetProperty("caseFailure", out _))
            {
                outcomes.Add(new ClipEditorialMetadataBatchOutcome(
                    Draft: null,
                    Failure: ParseCaseFailure(
                        result,
                        request,
                        outputSchema)));
                continue;
            ...
            if (Qwen3VlGroundedMetadataSchemaCapabilities
                    .SupportsSeparatedTitleTags(outputSchema) &&
                title.Contains('#'))
            {
                throw new Qwen3VlOutputParseException(
                    "Grounded Qwen metadata title must remain hashtag-free; " +
                    "game identity belongs in separate generated tags.");
            }
                    ...
                    EarliestOutputSchema))
            {
                ...
                validation = Qwen3VlGroundedMetadataRerollDiversityPolicy
                    .ReconcileReportedProvenance(
                        diversity,
                        validation,
                        retainReviewableMismatch:
                            Qwen3VlGroundedMetadataSchemaCapabilities
                                .SupportsEditorialFraming(outputSchema));
            }
            title = FinalizeAudienceTitle(title, request, outputSchema);
    ...
    internal static string FinalizeAudienceTitle(
        string titleBody,
        ClipEditorialMetadataRequest request,
        string outputSchema)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(titleBody);
        ArgumentNullException.ThrowIfNull(request);
        ArgumentException.ThrowIfNullOrWhiteSpace(outputSchema);
        string normalized = titleBody.Trim();
        if (!Qwen3VlGroundedMetadataSchemaCapabilities
                .SupportsSeparatedTitleTags(outputSchema))
        {
            return normalized;
        ...
        {
            throw new Qwen3VlOutputParseException(
                "Grounded Qwen metadata title body must remain hashtag-free before finalization.");
        }
        string suffix = " " +
        ...
        {
            throw new Qwen3VlOutputParseException(
                "Grounded Qwen metadata title body left no room for the canonical game hashtag.");
        }

        return normalized + suffix;
    }

    internal static ClipEditorialMetadataCaseFailure ParseCaseFailure(
        JsonElement result,
        ClipEditorialMetadataRequest request,
        string outputSchema)
    {
        if (!Qwen3VlGroundedMetadataSchemaCapabilities
                .SupportsPerCaseFailure(outputSchema))
        {
            throw new Qwen3VlOutputParseException(
                "Grounded Qwen metadata emitted a per-case failure under an unsupported schema.");
        }
        Qwen3VlEditorialJson.Exact(
            result,
            "candidateId",
            ...
            "caseFailure");
        ValidateResultIdentity(result, request);
        JsonElement failure = Qwen3VlEditorialJson.Object(
            result,
            "caseFailure");
        Qwen3VlEditorialJson.Exact(failure, "errorCode");
        ClipEditorialMetadataCaseFailureCode code =
            Qwen3VlEditorialJson.Text(failure, "errorCode") switch
            {
                "NoDistinctPrimaryVisualEvent" =>
                    ClipEditorialMetadataCaseFailureCode
                        .NoDistinctPrimaryVisualEvent,
                _ => throw new Qwen3VlOutputParseException(
                    "Grounded Qwen metadata emitted an unsupported per-case failure code."),
            };
        return new ClipEditorialMetadataCaseFailure(code);
    }
