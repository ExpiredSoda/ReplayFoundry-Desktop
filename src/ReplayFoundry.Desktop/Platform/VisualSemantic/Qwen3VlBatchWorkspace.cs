...
internal sealed class Qwen3VlBatchWorkspace
{
    ...
    private const int InitialCleanupRetryDelayMilliseconds = 50;
    ...
    public Exception? TryCleanup()
    {
        ...
        for (int attempt = 0; attempt < CleanupAttemptCount; attempt++)
        {
            try
            {
                      ...
                      SecurityException)
            {
                ...
                {
                    int delay =
                        InitialCleanupRetryDelayMilliseconds * (1 << attempt);
