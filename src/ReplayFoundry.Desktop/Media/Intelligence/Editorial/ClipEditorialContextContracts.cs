...
public sealed class ClipEditorialTranscriptContext
{
    ...
    public const int MaximumSpanCount = 32;
    private readonly ReadOnlyCollection<ClipEditorialTranscriptSpan> _spans;
        ...
        IReadOnlyList<ClipEditorialTranscriptSpan>? spans = null)
    {
        ...
        Text = text.Trim();
        Authority = authority;
        ClipEditorialTranscriptSpan[] spanSnapshot =
            spans?.ToArray() ?? [];
        if (spanSnapshot.Length > MaximumSpanCount ||
            spanSnapshot.Any(static value => value is null) ||
            spanSnapshot.Zip(
                    spanSnapshot.Skip(1),
                    static (left, right) =>
        ...
        {
            throw new ArgumentException(
                "Editorial transcript spans must be bounded, ordered, and non-overlapping.",
                nameof(spans));
        }
    ...
    public IReadOnlyList<ClipEditorialTranscriptSpan> Spans => _spans;
...
public sealed record ClipEditorialTranscriptSpan
{
    public const int MaximumTextLength = 500;

    public ClipEditorialTranscriptSpan(
        ...
        string text)
    {
        ...
        if (string.IsNullOrWhiteSpace(text) ||
            text.Trim().Length > MaximumTextLength)
        {
            throw new ArgumentException(
                $"A transcript span must contain at most {MaximumTextLength} characters.",
                nameof(text));
        }
...
public sealed class ClipEditorialGameContext
{
    ...
    public const string UnconfirmedGameName = "Gameplay";
    public const string UnconfirmedGameHashtag = "#Gameplay";
    ...
    public ClipEditorialGameContext(
        ...
        ConfirmedGameIdentity? confirmedIdentity = null)
    {
        if (string.IsNullOrWhiteSpace(gameName) ||
            gameName.Trim().Length > 120 ||
            string.IsNullOrWhiteSpace(gameHashtag) ||
            !gameHashtag.StartsWith('#') ||
            // One lexical title character plus one separating space must fit
            // beside the exact canonical hashtag in the 100-character
            // publishing title contract.
            gameHashtag.Length is < 2 or > 98 ||
        ...
        UseOpenGameKnowledge = isUserGrounded &&
    ...
    public string AudienceGameName => IsUserGrounded
        ? GameName
        : UnconfirmedGameName;

    public string AudienceGameHashtag => IsUserGrounded
        ? GameHashtag
        : UnconfirmedGameHashtag;
...
public sealed class ClipEditorialContext
{
        ...
        GroundedEditorialBrief? editorialBrief = null)
    {
        ...
        if (transcriptSnapshot.Any(static value => value is null) ||
            ...
            transcriptSnapshot.SelectMany(static value => value.Spans)
                .Any(span =>
        ...
        EditorialBrief = editorialBrief is null
            ? GroundedEditorialBriefBuilder.Build(
                CandidateId,
                SourceStart,
                SourceEnd,
                GameContext,
        ...
        if (!EditorialBrief.CandidateId.Equals(
                CandidateId,
                StringComparison.Ordinal) ||
            EditorialBrief.SourceStart != SourceStart ||
            EditorialBrief.SourceEnd != SourceEnd)
        {
            throw new ArgumentException(
                "The grounded editorial brief must belong to this exact candidate cut.",
                nameof(editorialBrief));
        }
    }
    ...
    public ClipEditorialContext WithSourceRange(
        ...
        new(
            ...
            DeterministicScore,
            DeterministicReason,
            TrimTranscripts(Transcripts, sourceStart, sourceEnd),
            Evidence,
            GameContext,
            GameKnowledge,
            GameplayRegion,
    ...
    private static IEnumerable<ClipEditorialTranscriptContext>
        TrimTranscripts(
            ...
            TimeSpan sourceEnd)
    {
        foreach (ClipEditorialTranscriptContext transcript in transcripts)
        {
            ...
            ClipEditorialTranscriptSpan[] retained = transcript.Spans
                .Where(span =>
                    span.SourceEnd > sourceStart &&
                    span.SourceStart < sourceEnd)
                .Select(span => new ClipEditorialTranscriptSpan(
                    span.SourceStart < sourceStart
                        ? sourceStart
                        : span.SourceStart,
                    span.SourceEnd > sourceEnd
            ...
            string text = string.Join(" ", retained.Select(static span => span.Text));
            ...
            yield return new ClipEditorialTranscriptContext(
