...
{
    public const int MaximumSourceRuleCodeLength = 80;
        ...
        string? sourceRuleCode = null)
    {
        ...
        SourceRuleCode = string.IsNullOrWhiteSpace(sourceRuleCode)
            ? null
            : sourceRuleCode.Trim().Length <= MaximumSourceRuleCodeLength
                ? sourceRuleCode.Trim()
                : throw new ArgumentException(
                    $"Editorial source rule codes cannot exceed {MaximumSourceRuleCodeLength} characters.",
    ...
    /// <summary>
    /// Stable provider or policy rule that produced this review finding.
    /// The user-facing message may change without changing retry or retention
    /// behavior.
    /// </summary>
    public string? SourceRuleCode { get; }
}
