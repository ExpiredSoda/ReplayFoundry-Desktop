...
public sealed class GenerationOutputProject
{
        ...
        StudioClipAppearance? appearance = null)
    {
        ...
        {
            GenerationSourceBindingPolicy.RequireCaptionTrack(
                captions,
                hidden.Id,
                hidden.SourceMedia,
                hidden.SourceStart,
        ...
        ClipEditorialContext? acceptedEditorialContext =
            editorialContext ?? hidden.EditorialContext;
        ClipEditorialMetadataDraft? acceptedEditorialMetadata =
            editorialMetadata ?? hidden.EditorialMetadata;
        if ((acceptedEditorialContext is null) !=
                (acceptedEditorialMetadata is null))
        {
            throw new ArgumentException(
                "Accepted Hidden Moment metadata must be complete and belong to the same candidate.",
                nameof(editorialContext));
        }
        if (acceptedEditorialContext is not null)
        {
            GenerationSourceBindingPolicy.RequireEditorialContext(
                acceptedEditorialContext,
                hidden.Id,
                hidden.SourceMedia,
                hidden.SourceStart,
                hidden.SourceEnd,
        ...
        StudioClipAppearance acceptedAppearance = appearance ??
            CaptionLook?.CreateAppearance() ??
        ...
        GenerationCandidateCaptionTrack? retainedCaptions =
            captions?.HasRenderableSegments == true
                ? captions
                    .WithRequestedStyle(acceptedAppearance.CaptionStyle)
                    .ToStudioHandoff()
                : null;
    ...
    private static StudioCaptionLook? ResolveCaptionLook(
        ...
        .Where(static item => item.Asset.HasCaptions)
        .Select(static item => new
        {
            Look = StudioCaptionLook.FromAppearance(item.Asset.Appearance),
            item.Index,
        })
        .GroupBy(static item => item.Look)
        ...
        .Select(static group => group.Look)
        .FirstOrDefault();
