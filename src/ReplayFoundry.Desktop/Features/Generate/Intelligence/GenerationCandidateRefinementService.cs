    ...
    IGenerationCandidateRefinementService
{
        ...
        GenerationVisualSemanticCandidateObservation reviewed)
    {
        ...
        [
            ...
            new GenerationCandidateRefinementComponent(
                ...
                0,
                qualifiedAction > 0
                    ? "The qualified picture review confirmed a distinct, self-contained action event."
                    : "The qualified picture review did not confirm a self-contained action event.",
                references),
        ];
        ...
        GenerationCandidateNaturalEndingAdjustment naturalEnding)
    {
        ...
        SpeechActivityInterval[] crossingEnd = allIntervals
            ...
            .ToArray();
        bool hasIncompleteSpeechEnding = crossingEnd.Length > 0 ||
            naturalEnding.RequiresAutomaticRejection;
        double incompleteSpeechEnding = hasIncompleteSpeechEnding ? 1 : 0;
        string[] endingReferences = crossingEnd.Select(interval =>
                $"vad:cut-end:{interval.AbsoluteStart:c}-{interval.AbsoluteEnd:c}")
            .Concat(naturalEnding.EvidenceReferences)
            .Distinct(StringComparer.Ordinal)
            .ToArray();
        (double creatorWeight, double gameWeight, double unknownWeight) =
            emphasis switch
            {
                ContentEmphasis.GameplayFocused => (-2d, 0d, 0d),
                ContentEmphasis.Balanced => (1.5d, 1d, 0d),
                ContentEmphasis.CommentaryFocused => (6d, 1d, 0.5d),
        ...
        return new GenerationCandidateRefinement(
            ...
            [
                new GenerationCandidateRefinementComponent(
                    ...
                    0,
                    all > 0
                        ? "Speech timing is retained for cut safety without automatically promoting this moment."
        ...
        GenerationSpeechActivityResult speechActivity)
    {
        ...
        foreach (GenerationSourceMomentResult source in moments.Sources)
        {
            ...
            SpeechActivityInterval[] creatorSpeech = sourceSpeech.Streams
                .Where(static stream =>
                    stream.Role.Role == AudioContentRole.CreatorSpeech)
                .SelectMany(static stream => stream.Intervals)
                .OrderBy(static interval => interval.AbsoluteStart)
                .ThenBy(static interval => interval.AbsoluteEnd)
                .ToArray();
            ...
            foreach (MomentCandidate candidate in source.Moments.Proposals)
            {
                bool preserveUserWindow = MatchesUserGuidance(
                    moments.Request,
                    source,
                    candidate);
            ...
            anyAdjusted = true;
            var repairedMediaMoments = new MediaMomentFindingResult(
                source.Moments.Request,
                source.Moments.Proposals.Select(candidate =>
                    replacements[candidate]),
                source.Moments.SelectedCandidates.Select(candidate =>
                    replacements[candidate]),
                source.Moments.Warnings,
                source.Moments.Manifest,
                source.Moments.ActivationSeries,
                source.Moments.Episodes);
            sources.Add(new GenerationSourceMomentResult(
                source.AnalyzedSource,
                repairedMediaMoments));
        }
        ...
        GenerationMomentCandidate[] selected = moments.SelectedCandidates
            .Select(existing =>
            {
                MomentCandidate replacement = replacements[existing.Candidate];
                GenerationCandidateRefinement? refinement =
                    existing.Refinement is null
                        ? null
                        : new GenerationCandidateRefinement(
                            replacement,
                            existing.Refinement.Components,
                            existing.Refinement.PolicyVersion);
                return new GenerationMomentCandidate(
                    existing.Id,
                    existing.AnalyzedSource,
                    replacement,
            ...
            .ToArray();
        var repairedMoments = new GenerationMomentFindingResult(
            moments.Request,
            sources,
            selected);
    ...
    private static bool MatchesUserGuidance(
        ...
        MomentCandidate candidate)
    {
        IReadOnlyList<UserMomentGuidance> guidance = request.Setup
            .MomentGuidance.ForSource(
                source.AnalyzedSource.PreparedSource.Media.FullPath);
        return guidance.Any(item => item.Kind switch
        {
            UserMomentGuidanceKind.PriorityPoint =>
                candidate.Window.Contains(item.Timestamp),
            UserMomentGuidanceKind.PriorityRange =>
                candidate.Window.Start < item.End &&
                candidate.Window.End > item.Start,
            _ => throw new ArgumentOutOfRangeException(nameof(item)),
        });
    }

    private sealed record PreparedNaturalEndings(
        GenerationMomentFindingResult Moments,
        IReadOnlyDictionary<
            MomentCandidate,
