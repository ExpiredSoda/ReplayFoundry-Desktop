            ...
            CancellationToken cancellationToken)
    {
        ...
        progress?.Report(
            new MomentEnrichmentProgressUpdate(
                ...
                "Planning which moments need a closer listen.",
                0,
                0));
            ...
            try
            {
                progress?.Report(
                    new MomentEnrichmentProgressUpdate(
                        MomentEnrichmentProgressPhase
                            .ExtractingAudio,
                        $"Preparing audio section {index + 1} of {plan.Neighborhoods.Count}.",
                        index + 1,
                        plan.Neighborhoods.Count));
                ...
                progress?.Report(
                    new MomentEnrichmentProgressUpdate(
                        MomentEnrichmentProgressPhase
                            .TranscribingAudio,
                        $"Listening to audio section {index + 1} of {plan.Neighborhoods.Count}.",
                        index + 1,
                        plan.Neighborhoods.Count));
        ...
        progress?.Report(
            new MomentEnrichmentProgressUpdate(
                MomentEnrichmentProgressPhase
                    .BindingCandidates,
                "Matching spoken words to the moments they belong to.",
                plan.Neighborhoods.Count,
                plan.Neighborhoods.Count));
        (
        ...
        progress?.Report(
            new MomentEnrichmentProgressUpdate(
                ...
                "Spoken words are ready for the selected moments.",
                plan.Neighborhoods.Count,
                plan.Neighborhoods.Count));
