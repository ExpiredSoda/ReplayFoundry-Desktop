...
public sealed class GenerationHiddenMoment
{
        ...
        GenerationCaptionStylePreset? captionStyle = null)
    {
        ...
        if (editorialMetadata is not null && editorialContext is null ||
        ...
        if (captionSourceSelection is not null)
        {
            GenerationSourceBindingPolicy.RequireCaptionSelection(
                captionSourceSelection,
                sourceMedia,
                nameof(captionSourceSelection));
        }
    ...
    public bool HasCompatibleEditorialMetadata =>
        EditorialMetadata is not null &&
        ClipEditorialMetadataGenerationPolicy.IsCompatible(
            EditorialPreference,
            EditorialMetadata);
    ...
    public GenerationHiddenMoment WithEditorialContext(
        ClipEditorialContext context)
    {
        ArgumentNullException.ThrowIfNull(context);
        if (!context.CandidateId.Equals(Id, StringComparison.Ordinal))
        {
            throw new ArgumentException(
                "Hidden-moment editorial context must belong to the same candidate.",
                nameof(context));
        }
        return HasGenerationProvenance
            ? new GenerationHiddenMoment(
                Id,
                ReviewOrder,
                SourceOrder,
                AnalyzedSource,
        ...
        GenerationCaptionStylePreset? captionStyle)
    {
        ...
        if (reviewOrder <= 0 || sourceOrder < 0 ||
            ...
            string.IsNullOrWhiteSpace(explanation) ||
            editorialMetadata is not null && editorialContext is null ||
            editorialMetadata is not null &&
            !ClipEditorialMetadataGenerationPolicy.IsCompatible(
                editorialPreference,
                editorialMetadata) ||
...
public static class GenerationHiddenMomentPlanner
{
    ...
    private static string Explain(CandidateEntry entry) =>
        ...
        entry.Candidate.Score.Components
            ...
            .Select(static value => value.Explanation)
            .FirstOrDefault() ??
        "Kept as a safe alternate based on the video scan.";
