...
public interface IGenerationGameKnowledgeService
{
    ...
    Task<ClipEditorialContext> RefreshAsync(
        ClipEditorialContext context,
        CancellationToken cancellationToken);
    ...
    Task RefreshAsync(
        ...
        Task.FromException(new NotSupportedException(
            "This game-knowledge service does not support identity-only refresh."));

    GameKnowledgeContextReceipt Inspect(ClipEditorialContext context);

    ClipEditorialContext RemoveCachedContext(ClipEditorialContext context);

    void RemoveCachedContext(ConfirmedGameIdentity identity);
}
    ...
    IGenerationGameKnowledgeService
{
    ...
    public static readonly TimeSpan OfflineFallbackFor = TimeSpan.FromDays(180);
    ...
    public async Task<ClipEditorialContext> EnrichAsync(
        ClipEditorialContext context,
        CancellationToken cancellationToken) =>
        await EnrichCoreAsync(context, forceRefresh: false, cancellationToken);

    public async Task<ClipEditorialContext> RefreshAsync(
        ClipEditorialContext context,
        CancellationToken cancellationToken) =>
        await EnrichCoreAsync(context, forceRefresh: true, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        GameKnowledgeCacheKey key = CacheKey(identity);
        GameKnowledgeSnapshot? existing = null;
        try
        {
            existing = _store.Find(key);
        }
        catch (InvalidDataException)
        {
            // The explicit refresh replaces a corrupt local cache only after
            // the official source completes successfully.
        }
        GameKnowledgeSnapshot refreshed = await _provider.AcquireAsync(
    ...
    public GameKnowledgeContextReceipt Inspect(ClipEditorialContext context) =>
        GameKnowledgeContextReceiptFactory.Create(
            context,
        ...
        ClipEditorialContext context)
    {
        ...
        if (context.GameContext.ConfirmedIdentity is
            ConfirmedGameIdentity identity)
        {
            RemoveCachedContext(identity);
        }
    ...
    {
        ArgumentNullException.ThrowIfNull(identity);
        _store.Remove(CacheKey(identity));
    }

    private async Task<ClipEditorialContext> EnrichCoreAsync(
        ClipEditorialContext context,
        bool forceRefresh,
        CancellationToken cancellationToken)
    {
        ...
        if (!context.GameContext.UseOpenGameKnowledge ||
            ...
            context.GameContext.ConfirmedIdentity is not
                ConfirmedGameIdentity identity)
        {
            return context;
        }
        ...
        try
        {
            ...
            if (forceRefresh || NeedsRefresh(snapshot, nowUtc))
            {
        ...
        {
            if (CanUseOfflineFallback(snapshot, DateTimeOffset.UtcNow))
            {
                return context.WithGameKnowledge(
                    _retriever.Retrieve(snapshot!, context));
            }
                ...
                new ClipGameKnowledgeContext(
                    ...
                    [
                        new GameKnowledgeWarning(
                            ...
                            "Public game information was unavailable, so the title and description use only the clip and details you entered."),
                    ]));
        }
    }

    private GameKnowledgeCacheKey CacheKey(ConfirmedGameIdentity identity) =>
        new(
            identity,
            _provider.Identity,
    ...
    private bool NeedsRefresh(
        ...
        !snapshot.Provider.Version.Equals(
            ...
            StringComparison.Ordinal) ||
        nowUtc < snapshot.RetrievedAtUtc ||
    ...
    private static bool CanUseOfflineFallback(
        ...
        nowUtc >= snapshot.RetrievedAtUtc &&
        nowUtc - snapshot.RetrievedAtUtc <= OfflineFallbackFor;
}
