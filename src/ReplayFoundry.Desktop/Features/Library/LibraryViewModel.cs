...
public sealed class LibraryViewModel : ObservableObject, IWorkspaceChromeSource, IDisposable
{
    ...
    private readonly LibraryAssetAvailabilityCoordinator
    ...
    private IReadOnlyList<LibraryGridEntry> _gridEntries = [];
    ...
    public LibraryViewModel()
        : this(
            ...
            fileAvailabilityMonitor: null,
            thumbnailRecoveryService: null)
    {
    }
    ...
    public LibraryViewModel(ILibraryCatalog catalog)
        : this(
            ...
            removalConfirmation: null,
            new LibraryFileAvailabilityMonitor(),
            thumbnailRecoveryService: null)
    {
    }
    ...
    internal LibraryViewModel(
        ...
        : this(
            ...
            thumbnailRecoveryService ??
                throw new ArgumentNullException(nameof(thumbnailRecoveryService)))
    {
    }
    ...
    internal LibraryViewModel(
        ...
        ILibraryThumbnailRecoveryService? thumbnailRecoveryService = null)
        : this(
            catalog ?? throw new ArgumentNullException(nameof(catalog)),
            catalog.Assets.Count > 0
                ? WorkspaceSurfaceState.ContentReady
            ...
            catalog as ILibraryAssetRemover,
            removalConfirmation: null,
            fileAvailabilityMonitor ??
                throw new ArgumentNullException(nameof(fileAvailabilityMonitor)),
            thumbnailRecoveryService)
    {
    }
        ...
        ILibraryThumbnailRecoveryService? thumbnailRecoveryService)
    {
        ...
        _availabilityCoordinator = new LibraryAssetAvailabilityCoordinator(
            fileAvailabilityMonitor,
            thumbnailRecoveryService,
        ...
        _availabilityCoordinator.Start(_catalog.Assets);
        _availabilityCoordinator.QueueMissingThumbnailRecoveries(_allItems);
    }
    ...
    public IReadOnlyList<LibraryGridEntry> GridEntries => _gridEntries;
    ...
    public void Dispose()
    {
        ...
        _isDisposed = true;
        _catalog.Changed -= Catalog_Changed;
        _availabilityCoordinator.Dispose();
    }
    ...
    private void Catalog_Changed(object? sender, EventArgs e)
    {
        ...
        _allItems = BuildItems(_catalog.Assets);
        _availabilityCoordinator.Watch(_catalog.Assets);
        ...
        LibraryFilesChangedEventArgs change)
    {
        ...
        string? selectedId = SelectedItem?.Asset?.Id;
        bool selectedMediaTouched =
            change.Touches(SelectedItem?.Asset?.OutputFullPath);
        ...
        foreach (LibraryItem item in _allItems)
        {
            ...
            bool mediaTouched = change.Touches(asset.OutputFullPath);
            bool thumbnailTouched = change.Touches(asset.ThumbnailFullPath);
            if (!mediaTouched && !thumbnailTouched)
            {
                continue;
            }

            statusChanged |= item.RefreshFileAvailability(
                thumbnailTouched);
            if (mediaTouched)
            {
                _availabilityCoordinator.QueueMissingThumbnailRecovery(item);
            }
        }
        ...
        if (selectedMediaTouched &&
            selectedId is not null &&
            SelectedItem?.Asset?.Id.Equals(
                selectedId,
                StringComparison.Ordinal) == true)
        {
            Playback.Load(SelectedItem);
        }
    ...
    private void RelinkMissingFile()
    {
        ...
        try
        {
            ...
            SetLibraryNotice(
                "The moved clip is linked again. Its Library title and details did not change.");
        }
        catch (Exception exception) when (
            exception is IOException or
            UnauthorizedAccessException or
    ...
    private static IReadOnlyList<LibraryItem> BuildItems(
        ...
        assets.Select(asset => new LibraryItem(
                ...
                asset.Mode == GenerationMode.Montage
                    ? $"{asset.ContributingCandidateCount} selected moments"
                    : "Finished Studio file",
                asset.Mode == GenerationMode.Montage
                    ? "Icon.Grid"
                    : "Icon.Spark",
