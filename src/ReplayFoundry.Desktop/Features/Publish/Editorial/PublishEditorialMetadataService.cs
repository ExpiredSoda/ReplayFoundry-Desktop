        ...
        CancellationToken cancellationToken)
    {
        ...
        GenerationOutputAsset source = Resolve(asset) ??
            throw new InvalidOperationException(
                "Replay Foundry no longer has enough saved information to " +
                "write another version for this Library video. You can still " +
                "edit and save its title, description, and tags.");
        ...
        return new PublishEditorialRerollResult(
            ...
            rerolled.QualityIssues.Count > 0
                ? "A new local-AI version is ready, but it may need edits. Review it or make another version before uploading."
                : rerolled.Origin == ClipEditorialMetadataOrigin.AiAssisted
                ? "A new local-AI version is ready. Review the title, description, and tags before uploading."
                : "A new title and description are ready. Review them before uploading.");
    }
