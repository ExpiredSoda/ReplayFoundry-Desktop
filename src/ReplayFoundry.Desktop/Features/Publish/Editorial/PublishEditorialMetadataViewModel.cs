    ...
    IDisposable
{
    ...
    private readonly AsyncOperationLifetime _commandOperations = new();
    ...
    private string _status =
        "Edit the prepared wording directly, or make a different version from what happens in this video.";
    ...
    public string AvailabilityText => CanRerollSelected
        ? UsesLocalAiForRerolls
            ? IsAiAvailable
                ? "Ready to make a genuinely different title and description from this video."
                : "Local AI needs attention before it can make another version."
            : "Ready to make a quick alternate version on this PC."
        : "This video does not have the saved clip details needed to make another version. You can still edit and save every field.";

    public string RerollButtonText => "Make a different version";

    public string RerollAutomationName => UsesLocalAiForRerolls
        ? "Create another YouTube title and description with local AI"
        : "Create another YouTube title and description with the built-in writer";
    ...
    {
        Task commandCompletion = _commandOperations.Seal();
        ...
        await Task.WhenAll(completion, commandCompletion)
    ...
    private async Task RerollAsync()
    {
        ...
        var rerollCancellation = new CancellationTokenSource();
        var rerollCompletion = new TaskCompletionSource<bool>(
            TaskCreationOptions.RunContinuationsAsynchronously);
        lock (_operationSync)
        {
            if (_isDisposed || _isStopping)
            {
                rerollCancellation.Dispose();
                return;
            }
            _cancellation = rerollCancellation;
            _completion = rerollCompletion;
        }
        _isGenerating = true;
        ...
        try
        {
            PublishEditorialRerollResult result = await _service.RerollAsync(
                ...
                _priorAcceptedTitles,
                requireAi,
                rerollCancellation.Token);
            rerollCancellation.Token.ThrowIfCancellationRequested();
            if (_asset is null || !_asset.Id.Equals(
                    selected.Id,
                    StringComparison.Ordinal))
            {
                _status =
                    "The selected Library video changed, so Replay Foundry discarded the older wording.";
                return;
            }
            if (!requestSnapshot.Equals(CaptureRerollSnapshot(selected.Id)))
            {
                _status =
                    "You changed this video's title, description, tags, or writing preferences while the new version " +
                    "was being created. Replay Foundry kept your edits and discarded the older result. " +
                    "Choose Make a different version again when you are ready.";
                return;
            }
            _applyDraft(result);
        ...
        catch (OperationCanceledException)
        {
            if (!_isDisposed)
            {
                _status = "Creating a new version was cancelled.";
            }
        }
        ...
        finally
        {
            try
            {
                lock (_operationSync)
                {
                    if (ReferenceEquals(_cancellation, rerollCancellation))
                    {
                        _cancellation = null;
                    }
                }
            ...
            {
                rerollCompletion.TrySetResult(true);
                lock (_operationSync)
                {
                    if (ReferenceEquals(_completion, rerollCompletion))
                    {
                        _completion = null;
                    }
                }
    ...
    private static void TryCancel(CancellationTokenSource? cancellation)
    {
        ...
        {
            // The reroll completed between capture and cancellation.
        }
    }
