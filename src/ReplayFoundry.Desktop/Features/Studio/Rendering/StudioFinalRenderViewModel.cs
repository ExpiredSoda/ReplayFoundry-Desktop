    ...
    IDisposable
{
    ...
    private TaskCompletionSource<bool>? _activeRenderCompletion;
    private GenerationOutputProject? _project;
    private string? _boundProjectId;
    private bool _isRendering;
    private bool _isStopping;
    private string _status =
        "No clips are queued. Choose Add to queue on a clip.";
    ...
    public StudioFinalRenderViewModel(
        ...
        Func<bool>? isPendingMetadataValid = null,
        Func<bool>? commitPendingMetadata = null,
    ...
    public string QueueSummary
    {
        get
        {
            ...
            if (_queue.Count == 0)
            {
                return "The queue is empty. Choose Add to queue on a clip.";
            }
            ...
            return pending == 0
                ? $"{completed} " +
                  ...
                  " a Library copy. Remove one or choose re-render for another copy."
                : $"{pending} " +
    ...
    public bool CanAttemptQueueAsset(string? assetId) =>
        CanAttemptQueueChange() &&
        assetId is not null &&
        _project?.Assets.FirstOrDefault(asset =>
            asset.Id.Equals(assetId, StringComparison.Ordinal)) is
            { IsIncludedInFinalRender: true } &&
        !_queue.Any(item => item.AssetId.Equals(
            assetId,
            StringComparison.Ordinal));

    public bool CanQueueAsset(string? assetId)
    {
        if (!CanAttemptQueueAsset(assetId) ||
            _project?.Assets.FirstOrDefault(asset =>
                asset.Id.Equals(assetId!, StringComparison.Ordinal)) is not
                { IsIncludedInFinalRender: true } asset ||
            NeedsValidClipEdit)
        {
            return false;
        }

        return asset.Id.Equals(
                   _selectedAsset()?.Id,
                   StringComparison.Ordinal) &&
               _hasUnsavedMetadata()
            ? _isPendingMetadataValid()
            : HasStructurallyValidMetadata(asset);
    }
    ...
    public bool NeedsMetadataFix =>
        ...
        (_hasUnsavedMetadata()
            ? !_isPendingMetadataValid()
            : !HasStructurallyValidMetadata(selected));
    ...
    public string ButtonText => _project switch
    {
        ...
        _ when FindSelectedIncludedAsset() is null => "Select a clip",
        _ when IsSelectedAssetQueued => "Queued",
        _ => "Add to queue",
    };
    ...
    public string ReadinessText => _project switch
    {
        ...
        _ when NeedsMetadataSave =>
            "Add to queue will save your title and description changes",
        _ => "Ready to add",
    };
    ...
    public bool RemoveAssetFromQueue(string assetId)
    {
        ...
        _queue.Remove(item);
        _status =
            $"Removed {item.Title} from the queue. The clip is still in Studio.";
        NotifyProperties();
        return true;
    }
    ...
    internal async Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _isStopping = true;
            activeCancellation = _cancellation;
            completion = _activeRenderCompletion?.Task ??
                Task.CompletedTask;
        }
    ...
    private bool CanAddToQueue() =>
        FindSelectedIncludedAsset() is { } selected &&
        CanQueueAsset(selected.Id);

    private bool CanAttemptAddToQueue() =>
        FindSelectedIncludedAsset() is { } selected &&
        CanAttemptQueueAsset(selected.Id);

    private void AddSelectedAssetToQueue()
    {
        if (!CanAttemptAddToQueue() ||
            _project is null ||
        ...
        {
            _status =
                "Move the clip end after its start before adding it.";
            NotifyProperties();
            return;
        }

        if (!TryCommitPendingMetadata())
        {
            _status =
                "Fix the title and description before adding this clip.";
            NotifyProperties();
            return;
        }
        ...
        if (currentSelected is not { IsIncludedInFinalRender: true } ||
            _queue.Any(item => item.AssetId.Equals(
             selectedAssetId,
             StringComparison.Ordinal)) ||
            !HasStructurallyValidMetadata(currentSelected))
        {
            _status = currentSelected is not null &&
                      !HasStructurallyValidMetadata(currentSelected)
                ? "Add a title and description before queueing this clip."
        ...
        _status = hasLibraryCopy
            ...
            : $"Added {currentSelected.EditorialMetadata?.Title ?? $"Clip {currentSelected.Rank:00}"} to the queue.";
        NotifyProperties();
    }
    ...
    private static bool HasStructurallyValidMetadata(
        GenerationOutputAsset asset) =>
        asset.EditorialMetadata?.IsPublishReady == true;

    private bool TryCommitPendingMetadata() =>
        !_hasUnsavedMetadata() ||
        _isPendingMetadataValid() && _commitPendingMetadata();
    ...
    private async Task RenderQueueAsync()
    {
        ...
        {
            _status =
                "Fix the title and description before rendering this queue.";
            NotifyProperties();
            return;
        }
        RefreshQueueItems();
        ...
        lock (_operationSync)
        {
            ...
            _activeRenderCompletion = renderCompletion;
        }
        SetRendering(true);
        _error = null;
        _percent = 0;
        ...
        finally
        {
            lock (_operationSync)
            {
                if (ReferenceEquals(_cancellation, renderCancellation))
                {
                    _cancellation = null;
                }
                if (ReferenceEquals(
                        _activeRenderCompletion,
                        renderCompletion))
                {
                    _activeRenderCompletion = null;
                }
            }
    ...
    private static void TryCancel(CancellationTokenSource? cancellation)
    {
        ...
        {
            // Rendering completed between capture and cancellation.
        }
    }
        ...
        new(
            ...
            isCompleted
                ...
                : hasLibraryCopy
                    ? "Ready · copy exists"
                    : "Ready",
            asset.HasApprovedEditorialMetadata
                ? "Title reviewed"
                : "Title ready",
            isCompleted,
            hasLibraryCopy,
