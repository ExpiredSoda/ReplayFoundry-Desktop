...
/// <summary>
/// The caption-only part of a Studio clip appearance. Keeping this separate
/// from video effects and graphics lets a project carry one established
/// caption look forward to clips that are added later.
/// </summary>
public sealed record StudioCaptionLook
{
    public StudioCaptionLook(
    ...
    {
        ArgumentNullException.ThrowIfNull(appearance);
        return new StudioClipAppearance(
            CaptionStyle,
            CaptionVerticalPositionPercent,
    ...
    public StudioClipAppearance CreateAppearance() => ApplyTo(
        StudioClipAppearance.CreateDefault(CaptionStyle));
}
