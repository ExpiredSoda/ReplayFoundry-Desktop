...
public sealed class StudioClipEditorViewModel : INotifyPropertyChanged
{
    ...
    private readonly DelegateCommand _applyCaptionLookToAllCommand;
    ...
    public StudioClipEditorViewModel(IGenerationOutputEditor? outputEditor)
    {
        ...
        _resetCommand = new DelegateCommand(
            ResetDraft,
            CanResetDraft);
        _applyCaptionLookToAllCommand = new DelegateCommand(
            ApplyCaptionLookToAll,
            CanApplyCaptionLookToAll);
    ...
    public string BoundaryPrecisionText =>
        "Arrow keys make tiny changes · Page Up or Down moves one second";
    ...
    public string ApplyCaptionLookToAllText => CaptionedClipCount switch
    {
        ...
        int count => $"Apply this caption look to all {count} captioned clips",
    };
    public ICommand ApplyCaptionLookToAllCommand =>
        _applyCaptionLookToAllCommand;
    ...
    private void ApplyBoundaryEdit()
    {
            ...
            _outputEditor is null)
        {
            throw new InvalidOperationException(
                "The start and end changes are not ready to apply.");
        }
        ...
        _error = null;
        _status =
            "Changes saved. Render the queue when you are ready to create the finished files.";
        NotifyDraftProperties();
    }

    private bool CanApplyCaptionLookToAll() =>
    ...
    {
        if (!CanApplyCaptionLookToAll() ||
            _project is null ||
            _outputEditor is null)
        {
            throw new InvalidOperationException(
                "The caption look cannot be applied across this Studio project.");
        }

        var captionLook = new StudioCaptionLook(
            SelectedCaptionStyle.Value,
            CaptionVerticalPositionPercent,
            SelectedCaptionWordLimit.Value,
            CaptionMaximumWidthPercent,
        ...
        GenerationOutputAsset[] replacements = _project.Assets
            ...
            .Select(asset => asset.WithStudioEdits(
                asset.SourceStart,
                asset.SourceEnd,
                captionLook.ApplyTo(asset.Appearance)))
            .ToArray();
        _outputEditor.ReplaceAssets(_project.Id, replacements);
        _error = null;
        _status =
            $"Caption look copied to all {replacements.Length} captioned clips. Each clip kept its own words and timing.";
        NotifyDraftProperties();
    }
    ...
    private void ResetDraft()
    {
        ...
        _error = null;
        _status =
            "Restored the saved look and suggested start and end.";
        NotifyDraftProperties();
        DraftAppearanceChanged?.Invoke(this, EventArgs.Empty);
    }
