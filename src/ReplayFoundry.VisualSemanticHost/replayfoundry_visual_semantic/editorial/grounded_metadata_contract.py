...
from .grounded_metadata_editorial_brief_contract import (
    EDITORIAL_BRIEF_COPY_GOAL,
    EDITORIAL_BRIEF_POLICY_VERSION,
    validate_editorial_brief as _validate_editorial_brief,
)
...
GROUNDED_MAXIMUM_REVIEW_DURATION_SECONDS = Decimal("180")
...
def validate_request(
    ...
    if "editorialBrief" in request:
        request_keys.add("editorialBrief")
    _require_exact_keys(
        request,
        request_keys,
        location,
    ...
    validated_video = _validate_review_video(
        ...
        media_hash_cache,
        GROUNDED_MAXIMUM_REVIEW_DURATION_SECONDS,
    )
    ...
    for transcript_index, transcript_value in enumerate(transcripts_value):
        ...
        if "spans" in transcript:
            transcript_keys.add("spans")
        _require_exact_keys(
            transcript,
            transcript_keys,
            transcript_location,
        ...
        last_end = start
        for span_index, span_value in enumerate(_require_array(
            transcript.get("spans", []),
            f"{transcript_location}.spans",
            maximum=32,
        )):
            span_location = f"{transcript_location}.spans[{span_index}]"
            span = _require_object(span_value, span_location)
            _require_exact_keys(
                span, {"startSeconds", "endSeconds", "text"}, span_location
            )
            span_start = _finite_number(
                span["startSeconds"], f"{span_location}.startSeconds", start, end
            )
            span_end = _finite_number(
                span["endSeconds"], f"{span_location}.endSeconds", start, end
            )
            if span_end <= span_start or span_start < last_end:
                _fail(UsageOrInputError, f"{span_location} is not ordered and bounded.")
            last_end = span_end
            spans.append({
                "startSeconds": span_start,
                "endSeconds": span_end,
                "text": bounded_text(span["text"], f"{span_location}.text", 500),
            })
        transcripts.append(
            {
    ...
    editorial_brief = _validate_editorial_brief(
        request.get("editorialBrief"),
        f"{location}.editorialBrief",
    )
