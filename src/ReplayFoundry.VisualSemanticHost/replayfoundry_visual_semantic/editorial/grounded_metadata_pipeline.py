...
def _prepare_synthesis_context(
    ...
    editorial_frame = dict(facts["editorialFrame"])
    assessments = facts["visualEventSelectionAssessments"]
    primary_index = facts["primaryVisualDraftOrdinal"] - 1
    if 0 <= primary_index < len(assessments):
        editorial_frame["primaryPresentationKind"] = assessments[
            primary_index
        ]["presentationKind"]
    supporting_ordinals = {
        ordinal
        for ordinal in editorial_frame.get("supportingDraftOrdinals", [])
        if isinstance(ordinal, int)
        and not isinstance(ordinal, bool)
        and 1 <= ordinal <= len(assessments)
    }
    editorial_frame["supportingPresentationKinds"] = list(dict.fromkeys(
        assessments[ordinal - 1]["presentationKind"]
    ...
    return SynthesisContext(
        ...
        editorial_frame=facts["editorialFrame"],
