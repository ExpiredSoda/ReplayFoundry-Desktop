...
def _request_with_selected_knowledge(
    ...
    if knowledge is None:
        ...
        result["editorialBrief"] = _brief_with_selected_knowledge(
    ...
    if selected_identity == "None":
        ...
        result["editorialBrief"] = _brief_with_selected_knowledge(
            request.get("editorialBrief"),
            frozenset(item["id"] for item in general_context),
        )
        return result
    selected_index = next(
    ...
    result["editorialBrief"] = _brief_with_selected_knowledge(
        request.get("editorialBrief"),
        frozenset(item["id"] for item in retained),
    )
    return result


def _brief_with_selected_knowledge(
    value: dict[str, Any] | None,
    retained_knowledge_ids: frozenset[str],
) -> dict[str, Any] | None:
    """Remove hypotheses and unselected public passages before prose synthesis."""
    if value is None:
        return None
    knowledge_kinds = {
        "MissionOrChapter",
    ...
    retained_claims = [
        ...
        if claim["state"] in {"Confirmed", "Supported"}
        and (
            claim["kind"] not in knowledge_kinds
            or claim["id"] in retained_knowledge_ids
        )
    ]
    retained_claim_ids = frozenset(claim["id"] for claim in retained_claims)
    result = dict(value)
    result["claims"] = retained_claims
    result["sourceBindings"] = [
        dict(binding)
        for binding in value["sourceBindings"]
        if binding["claimId"] in retained_claim_ids
    ]
    return result
