...
_ACTION_STRENGTH_RULES = (
    (
        ...
        re.compile(
            r"\b(?:defeat(?:s|ed|ing)?|kill(?:s|ed|ing)?|"
            r"destroy(?:s|ed|ing)?|eliminat(?:e|es|ed|ing)|"
            r"vanquish(?:es|ed|ing)?|win(?:s|ning)?|won|"
            r"die|dies|died|dying|collaps(?:e|es|ed|ing)|health bar "
            r"(?:is |was )?(?:empt(?:y|ies|ied|ying)|reach(?:es|ed|ing)? zero))\b",
            re.IGNORECASE,
        ),
    ),
    (
        ...
        re.compile(
            r"\b(?:enter(?:s|ed|ing)?|pass(?:es|ed|ing)? through|"
            r"mov(?:e|es|ed|ing) (?:into|through)|"
            r"walk(?:s|ed|ing)? (?:into|through)|"
            r"(?:run|runs|ran|running) (?:into|through))\b",
            re.IGNORECASE,
        ),
    ),
    (
        ...
        re.compile(
            r"\b(?:explod(?:e|es|ed|ing)|detonat(?:e|es|ed|ing)|"
            r"(?:blow|blows|blew|blowing) up|(?:burst|bursts|bursting) apart)\b",
            re.IGNORECASE,
        ),
    ),
    (
        ...
        re.compile(
            r"\b(?:disappear(?:s|ed|ing)?|vanish(?:es|ed|ing)?|reappear"
            r"(?:s|ed|ing)?|rematerializ(?:e|es|ed|ing))\b",
            re.IGNORECASE,
        ),
    ),
    (
        ...
        re.compile(
            r"\b(?:complet(?:e|es|ed|ing)|finish(?:es|ed|ing)?|clear(?:s|ed|ing)?)\b",
            re.IGNORECASE,
        ),
    ),
)
...
_TITLE_SCOPE_MOMENTS = frozenset({
    "Action", "Progress", "Complication", "Discovery", "Exposition",
    "Decision", "Outcome",
})
_TITLE_SCOPE_PRESENTATIONS = frozenset({
    "CinematicSequence", "InWorldRecording", "DocumentOrLore",
    "MenuOrLoadout", "ObjectiveOrInterface", "MixedOrTransition",
})


def _title_scope_draft_ordinals(
    ...
    if primary_only_evidence:
        return [primary_ordinal]
    frame = request.get("_editorialFraming")
    if not isinstance(frame, dict) or frame.get("authorityKind") != "StoryShapeOnly":
        return [primary_ordinal]
    presentations = {
        value
        for value in (
    ...
    if (
        frame.get("momentKind") not in _TITLE_SCOPE_MOMENTS
        and not presentations.intersection(_TITLE_SCOPE_PRESENTATIONS)
    ):
        return [primary_ordinal]
    ordinals = {
        ordinal
        for ordinal in frame.get("supportingDraftOrdinals", [])
        if isinstance(ordinal, int)
        and not isinstance(ordinal, bool)
        and 1 <= ordinal <= draft_count
    }
    ordinals.add(primary_ordinal)
...
def validate_primary_title_scope(
    ...
    allowed_ordinals = set(_title_scope_draft_ordinals(
        request,
    ...
    primary_tokens = set().union(*(
        per_draft[ordinal - 1] for ordinal in allowed_ordinals
    ))
    repeated_tokens = (
        set()
        if primary_only_evidence
    ...
    visual_text_tokens = (
        set()
        if primary_only_evidence
        else _value_tokens(request.get("visualTextAnchors", []))
    )
    non_primary_tokens = (
        set().union(*(
            tokens
            for index, tokens in enumerate(per_draft)
            if index + 1 not in allowed_ordinals
        ))
        if len(per_draft) > 1
        else set()
    )
...
def validate_primary_action_entailment(
    title_body: str,
    description: str,
    title_scope_drafts: list[dict[str, Any]],
) -> None:
    """Reject audience outcomes stronger than the validated title-scope actions."""
    scoped_actions = " ".join(
        action
        for draft in title_scope_drafts
    ...
    for audience_field, audience_copy in (
        ...
        for audience_pattern, support_pattern in _ACTION_STRENGTH_RULES:
            ...
            if offending is None or support_pattern.search(scoped_actions):
                continue
