...
internal static partial class GenerationClipRenderingTests
{
    ...
    private static async Task PipelineFindsThenRenders()
    {
        ...
        TestAssert.True(
            result.EditorialMetadata is not null &&
            result.EditorialMetadata.Candidates.All(candidate =>
                candidate.Draft.Origin ==
                    ClipEditorialMetadataOrigin.Heuristic),
            "The explicitly no-AI pipeline fixture must still run its required metadata service instead of completing with missing copy.");
        TestAssert.Equal(
            0,
