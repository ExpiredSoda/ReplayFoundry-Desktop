...
internal static class EditorialRerollDiversityTests
{
    ...
    [
        ...
        new(
            "Reviewable Qwen rerolls retain typed negative diversity provenance",
            ReviewableNegativeDiversityProvenanceIsValid),
        new(
            "Qwen title completeness recognizes bounded verb-particle endings",
            QwenTitleCompletenessRecognizesVerbParticles),
    ];
    ...
    private static Task ReviewableNegativeDiversityProvenanceIsValid()
    {
        (bool Expected, int Count,
            Qwen3VlGroundedMetadataRerollTitleDiversityCode Code,
            int Numerator, int Denominator, string Message)[] cases =
        [
            (true, 2, Qwen3VlGroundedMetadataRerollTitleDiversityCode.HighTokenOverlap, 1, 1,
                "A reordered exact token set remains reviewable high overlap."),
            (true, 1, Qwen3VlGroundedMetadataRerollTitleDiversityCode.ExactCanonicalTitle, 1, 1,
                "An exact reviewable title retains typed provenance."),
            (true, 0, Qwen3VlGroundedMetadataRerollTitleDiversityCode.NoComparablePrior, 0, 1,
                "A first title retains canonical no-comparison provenance."),
            (true, 1, Qwen3VlGroundedMetadataRerollTitleDiversityCode.HighTokenOverlap, 17, 20,
                "The exact similarity threshold belongs to high overlap."),
            (true, 1, Qwen3VlGroundedMetadataRerollTitleDiversityCode.MateriallyDistinct, 84, 100,
                "A result below the threshold is materially distinct."),
            (false, 1, Qwen3VlGroundedMetadataRerollTitleDiversityCode.MateriallyDistinct, 17, 20,
                "The threshold cannot claim material distinction."),
            (false, 1, Qwen3VlGroundedMetadataRerollTitleDiversityCode.HighTokenOverlap, 84, 100,
                "A below-threshold result cannot claim high overlap."),
            (false, 1, Qwen3VlGroundedMetadataRerollTitleDiversityCode.NoComparablePrior, 0, 1,
                "A retained prior title cannot report no comparison."),
            (false, 0, Qwen3VlGroundedMetadataRerollTitleDiversityCode.HighTokenOverlap, 1, 1,
                "Negative diversity requires a comparable prior title."),
        ];
    ...
    private static Task QwenTitleCompletenessRecognizesVerbParticles()
    {
        foreach (string title in new[]
                 {
                     "Control's title screen fades in #Control",
                     "The field agent checked in #Control",
                     "Jesse logged in #Control",
                 })
        {
            TestAssert.False(
                Qwen3VlGroundedMetadataLanguageRules.HasDanglingTitleEnding(
                    title,
                    "#Control"),
                $"A complete bounded verb-particle ending must remain usable: {title}");
        }

        foreach (string title in new[]
                 {
                     "Jesse walks in the #Control",
                     "Jesse enters through #Control",
                     "The signal drifted in #Control",
                 })
        {
            TestAssert.True(
                Qwen3VlGroundedMetadataLanguageRules.HasDanglingTitleEnding(
                    title,
                    "#Control"),
                $"An unrecognized or genuinely dangling ending must remain flagged: {title}");
        }
