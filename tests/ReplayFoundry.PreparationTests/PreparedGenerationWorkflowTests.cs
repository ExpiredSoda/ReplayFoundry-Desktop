...
internal static class PreparedGenerationWorkflowTests
{
    ...
    [
        ...
        new(
            "Generation preflight requires editorial AI only for AI authoring",
            PreflightRequiresEditorialAiOnlyForAiAuthoring),
        new(
    ...
    private static Task PreflightRequiresEditorialAiOnlyForAiAuthoring()
    {
        GenerationSourcePreparationResult preparation =
            CreatePreparation(
            [
                (
                    TestMediaFactory.CreateSourcePath(
                        "editorial-ai-preflight.mkv"),
                    true,
                    true),
            ]);
        GenerationCompositionReviewResult composition =
            CreateCompositionReview(preparation);
        const string repairReason =
            "Safe editorial AI repair guidance.";
        var unavailableCapabilities =
            new GenerationRuntimeCapabilities(
                ...
                EditorialAiUnavailableReason: repairReason);

        GenerationEngineUnavailableException exception =
            TestAssert.Throws<GenerationEngineUnavailableException>(
                ...
                "AI-required metadata should fail before evidence analysis when the provider is not ready.");
        TestAssert.Equal(
            repairReason,
            exception.Message,
            "Preflight should preserve the safe composition failure reason.");
