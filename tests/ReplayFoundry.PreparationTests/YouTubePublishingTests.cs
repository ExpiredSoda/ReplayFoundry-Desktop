...
internal static class YouTubePublishingTests
{
    ...
    [
        ...
        new("YouTube history dashboard keeps five newest records without trimming storage", HistoryDashboardIsBoundedWithoutTrimming),
        new("YouTube history archive filters sorts groups and pages the whole log", HistoryArchiveProjectsTheWholeLog),
        new("YouTube history archive uses an owned virtualized accessible surface", HistoryArchiveSurfaceIsBoundedAndAccessible),
        new("YouTube history links open only trusted YouTube pages", HistoryLinksAreTrusted),
        ...
        new("Publish calendar preserves the exact dropped date", CalendarPreservesExactDroppedDate),
        ...
        new("YouTube upload waits for a received transfer to finalize", UploadWaitsForFinalization),
        new("YouTube upload bounds missing finalization status", UploadBoundsMissingFinalization),
        new("YouTube terminal 308 waits for the finalized video id", TerminalResumeIncompleteWaitsForFinalization),
        new("YouTube terminal 308 bounds missing finalization", TerminalResumeIncompleteBoundsFinalization),
    ...
    private static Task HistoryDashboardIsBoundedWithoutTrimming()
    {
        DateTimeOffset now = new(
            2026, 9, 1, 16, 0, 0, TimeSpan.Zero);
        var store = new InMemoryYouTubePublishHistoryStore();
        for (int index = 0; index < 80; index++)
        {
            store.Append(CreateHistoryEntry(
                index,
                YouTubePublishOutcome.Published,
                now.AddMinutes(index)));
        }

        var history = new PublishHistoryViewModel(
            utcNow: () => now.AddHours(2),
            timeZone: TimeZoneInfo.Utc);
        history.Refresh(store.Current);

        TestAssert.Equal(
            80,
            store.Current.Count,
            "The local history store must retain every recorded upload rather than imposing a dashboard cap.");
        TestAssert.Equal(
    ...
    private static Task HistoryArchiveProjectsTheWholeLog()
    {
        DateTimeOffset now = new(
            2026, 9, 1, 16, 0, 0, TimeSpan.Zero);
        YouTubePublishHistoryEntry[] entries = Enumerable.Range(0, 57)
            .Select(index => index switch
            {
                0 => CreateHistoryEntry(
                    index,
                    YouTubePublishOutcome.Scheduled,
                    now.AddDays(-index)),
                1 => CreateHistoryEntry(
                    index,
                    YouTubePublishOutcome.Failed,
                    now.AddDays(-index)),
                2 => CreateHistoryEntry(
                    index,
                    YouTubePublishOutcome.Published,
                    now.AddDays(-index),
        ...
        var history = new PublishHistoryViewModel(
            utcNow: () => now,
            timeZone: TimeZoneInfo.Utc);
        history.Refresh(entries);

        TestAssert.Equal(
            PublishHistoryProjector.PageSize,
            history.PagedItems.Count,
            "The archive must present one bounded 25-record page first.");
        TestAssert.True(
            history.HasMoreItems,
            "More than one filtered page must expose a load-more action.");
        history.LoadMoreCommand.Execute(null);
        TestAssert.Equal(
            50,
            history.PagedItems.Count,
            "Load more must add exactly one more bounded page.");

        history.SelectedStatusFilter =
            PublishHistoryStatusFilter.NeedsAttention;
        TestAssert.Equal(
            2,
            history.FilteredCount,
            "Failed uploads and inaccessible remote videos must remain visible under Needs attention.");
        TestAssert.Equal(
            2,
            history.PagedItems.Count,
            "Changing a filter must reset and clamp the current page.");
        history.SelectedStatusFilter = PublishHistoryStatusFilter.Scheduled;
        TestAssert.Equal(
            1,
            history.FilteredCount,
            "Scheduled releases must remain a distinct status scope.");
        history.SelectedStatusFilter = PublishHistoryStatusFilter.All;
        history.SearchQuery = "Moment 42";
        TestAssert.Equal(
            1,
            history.FilteredCount,
            "Title search must update the archive projection immediately.");
        TestAssert.Equal(
            "history-42",
            history.PagedItems[0].Id,
            "Live title search must return the matching record.");

        history.ClearFiltersCommand.Execute(null);
        history.FromDate = now.Date.AddDays(-10);
        history.ToDate = now.Date.AddDays(-5);
        TestAssert.Equal(
            6,
            history.FilteredCount,
            "The inclusive date range must filter by local attempt date.");
        history.FromDate = now.Date;
        history.ToDate = now.Date.AddDays(-1);
        TestAssert.True(
            history.HasInvalidDateRange && history.PagedItems.Count == 0,
            "An inverted date range must be explained instead of silently swapping user input.");

        history.ClearFiltersCommand.Execute(null);
        history.SelectedSortOrder = PublishHistorySortOrder.OldestFirst;
        TestAssert.Equal(
            "history-56",
            history.PagedItems[0].Id,
            "Oldest-first sorting must apply across the filtered whole set before paging.");
        TestAssert.Equal(
            "TODAY",
            history.RecentItems[0].DateGroup,
            "Current-day activity must receive a meaningful date group.");
        return Task.CompletedTask;
    }

    private static Task HistoryArchiveSurfaceIsBoundedAndAccessible()
    {
        ...
        TestAssert.True(
            ...
            dashboard.Contains(
                "Content=\"View all history\"",
                StringComparison.Ordinal) &&
            !dashboard.Contains(
                "ItemsSource=\"{Binding HistoryItems}\"",
                StringComparison.Ordinal),
            "The Publish dashboard must show only its compact projection and a clear archive action.");
        TestAssert.True(
            archive.Contains(
            ...
            archive.Contains(
                ...
                StringComparison.Ordinal),
            "The owned history surface must use one bounded recycling list rather than an ever-growing dashboard stack.");
        TestAssert.True(
            archive.Contains(
                "PropertyName=\"DateGroup\"",
                StringComparison.Ordinal) &&
            archive.Contains(
                "AutomationProperties.Name=\"Filtered YouTube history\"",
                StringComparison.Ordinal) &&
            archive.Contains(
                "AutomationProperties.LiveSetting=\"Polite\"",
                StringComparison.Ordinal),
            "The archive must expose meaningful date groups and accessible dynamic results.");
        return Task.CompletedTask;
    }

    private static Task HistoryLinksAreTrusted()
    {
        var dialog = new RecordingHistoryDialog();
        var launcher = new RecordingHistoryLinkLauncher();
        DateTimeOffset now = new(
            2026, 9, 1, 16, 0, 0, TimeSpan.Zero);
        var history = new PublishHistoryViewModel(
            dialog,
        ...
        TestAssert.Equal(
            1,
            dialog.CallCount,
            "The dashboard archive action must open one focused owned surface.");
        history.OpenOnYouTubeCommand.Execute(history.PagedItems[0]);
        TestAssert.Equal(
            "https://www.youtube.com/watch?v=video-1",
            launcher.LastUri!.AbsoluteUri,
            "A recorded YouTube URL must open through the injected system-link boundary.");
        TestAssert.False(
            PublishHistoryViewModel.TryGetTrustedYouTubeUri(
                "https://example.com/watch?v=video-1",
                out _),
            "An arbitrary HTTPS page must never be accepted as a YouTube history action.");
        TestAssert.False(
            PublishHistoryViewModel.TryGetTrustedYouTubeUri(
                "http://youtube.com/watch?v=video-1",
                out _),
            "A non-HTTPS YouTube URL must never be opened from local history.");
        var untrustedRow = new PublishHistoryListItem(
            "history-untrusted",
            "Legacy record",
            "Published",
            "Completed locally",
        ...
        TestAssert.False(
            untrustedRow.HasUrl,
            "A legacy non-YouTube URL must not expose an unusable Open button.");
        return Task.CompletedTask;
    }
    ...
    private static async Task PublishEditorialStopWaitsForDraftPersistence()
    {
        using var fixture = new PublishAssetFixture(32);
        var service = new FakePublishEditorialMetadataService();
        var current = new PublishEditorialMetadataSnapshot(
        ...
        using var viewModel = new PublishEditorialMetadataViewModel(
            service,
            result => current = new PublishEditorialMetadataSnapshot(
                result.Title,
                result.Description,
                result.Tags),
            () => current,
            ...
            () => false);
        viewModel.BindAsset(fixture.Asset);

        Task reroll = Task.Run(() =>
            ((AsyncDelegateCommand)viewModel.RerollCommand).ExecuteAsync());
        Task stop = Task.CompletedTask;
        try
        {
            await persistenceStarted.Task.WaitAsync(TimeSpan.FromSeconds(5));
            stop = viewModel.StopAsync(CancellationToken.None);
            TestAssert.False(
                stop.IsCompleted,
                "Publish editorial stop must await the final draft write.");
        }
        finally
        {
            releasePersistence.Set();
            await Task.WhenAll(reroll, stop)
    ...
    private static Task CalendarRejectsPastDateDrops()
    {
        using var fixture = new PublishAssetFixture(32);
        var catalog = new FixedCatalog(fixture.Asset);
        var dialog = new RecordingPreparationDialog();
        ...
        using var publish = new PublishViewModel(
            ...
            TimeZoneInfo.Utc,
            preparationDialog: dialog);
        DateTime pastDate = now.Date.AddDays(-1);
        var pastDay = new PublishCalendarDay(
            pastDate,
            pastDate.Day.ToString(),
            pastDate.ToLongDateString(),
            true,
            false,
            []);

        PublishCalendarDropTargetState state =
            PublishCalendarView.ResolveDropTargetState(
                publish,
                pastDay,
                hasLibraryAsset: true);
        publish.PrepareAssetForDate(fixture.Asset, pastDate);

        TestAssert.Equal(
            PublishCalendarDropTargetState.RejectedPastDate,
            state,
            "A past calendar cell must advertise a rejected drop instead of a link operation.");
        TestAssert.Equal(
            0,
            dialog.CallCount,
            "A rejected past-date drop must not open the preparation dialog.");
        TestAssert.True(
            publish.HasCalendarDropFeedback &&
            publish.CalendarDropFeedback ==
                "That date has passed. Choose today or a later date.",
            "A rejected drop must expose concise calendar-local feedback.");
        TestAssert.Equal(
            string.Empty,
            publish.Notice,
            "Calendar drop feedback must not leak into the unopened preparation dialog.");
        ...
        TestAssert.True(
            xaml.Contains("IsDropTargetRejected", StringComparison.Ordinal) &&
            xaml.Contains("Text=\"DATE HAS PASSED\"", StringComparison.Ordinal) &&
            xaml.Contains("HasCalendarDropFeedback", StringComparison.Ordinal) &&
            xaml.Contains(
                "AutomationProperties.LiveSetting=\"Assertive\"",
                StringComparison.Ordinal),
            "The Publish calendar must pair its rejected-drop state with visible and accessible local feedback.");
        return Task.CompletedTask;
    }

    private static Task CalendarPreservesExactDroppedDate()
    {
        using var fixture = new PublishAssetFixture(32);
        var catalog = new FixedCatalog(fixture.Asset);
        var dialog = new RecordingPreparationDialog();
        DateTimeOffset now = new(
            ...
            0,
            TimeSpan.Zero);
        var preferences = new InMemoryYouTubePublishPreferencesStore();
        preferences.Replace(
        [
            new YouTubePreferredScheduleSlot(
                DayOfWeek.Wednesday,
                new TimeOnly(9, 30)),
        ]);
        using var publish = new PublishViewModel(
            ...
            TimeZoneInfo.Utc,
            preparationDialog: dialog);
        DateTime selectedDate = now.Date.AddDays(1);
        DateTime droppedDate = now.Date.AddDays(4);
        publish.SelectedCalendarDay = publish.CalendarDays.Single(day =>
            day.Date == selectedDate);
        publish.PrepareAssetForDate(fixture.Asset, now.Date.AddDays(-1));

        var droppedDay = new PublishCalendarDay(
            droppedDate,
            droppedDate.Day.ToString(),
            droppedDate.ToLongDateString(),
            true,
            false,
            []);
        PublishCalendarDropTargetState state =
            PublishCalendarView.ResolveDropTargetState(
                ...
                droppedDay,
                hasLibraryAsset: true);
        publish.PrepareAssetForDate(fixture.Asset, droppedDate);

        TestAssert.Equal(
            PublishCalendarDropTargetState.Accepted,
            state,
            "Today and future calendar cells must advertise a valid link drop.");
        TestAssert.Equal(
            droppedDate,
            publish.ScheduledDate!.Value,
            "The explicit drop target must win over both the selected day and preferred schedule slots.");
        TestAssert.Equal(
            1,
            dialog.CallCount,
            "One valid drop must open one preparation dialog.");
        TestAssert.False(
            publish.HasCalendarDropFeedback,
            "A valid drop must clear earlier rejected-drop feedback.");
        return Task.CompletedTask;
    }
    ...
    private static async Task UploadWaitsForFinalization()
    {
        using var fixture = new PublishAssetFixture(32);
        var handler = new FinalizingYouTubeHandler(finalizeOnStatusRequest: 3);
        using var client = new HttpClient(handler)
        {
        ...
        string id = await api.UploadVideoAsync(
            "token",
            fixture.CreateRequest(),
            null,
            CancellationToken.None);

        TestAssert.Equal("video-after-status", id,
            "A completed transfer should accept the eventual finalized video identifier.");
        TestAssert.Equal(3, handler.StatusRequests,
            "Finalization status polling should stop immediately when YouTube returns the identifier.");
        TestAssert.Equal(1, handler.DataRequests,
            "Finalization polling must not resend an empty or duplicate data chunk.");
    }

    private static async Task UploadBoundsMissingFinalization()
    {
        using var fixture = new PublishAssetFixture(32);
        var handler = new FinalizingYouTubeHandler(finalizeOnStatusRequest: null);
        using var client = new HttpClient(handler)
        {
            Timeout = Timeout.InfiniteTimeSpan,
        };
        ...
        YouTubePublishingException exception =
            await TestAssert.ThrowsAsync<YouTubePublishingException>(
                () => api.UploadVideoAsync(
                    "token",
                    fixture.CreateRequest(),
                    null,
                    CancellationToken.None),
                "A fully received transfer that never finalizes must fail after bounded status checks.");

        TestAssert.Equal(
            "youtube.upload.finalization-incomplete",
            exception.DiagnosticCode,
            "Missing finalization should retain a stable diagnostic code.");
        TestAssert.Equal(3, handler.StatusRequests,
            "Finalization polling must use the fixed bounded attempt count.");
        TestAssert.Equal(1, handler.DataRequests,
            "A full remote offset must never produce a zero-length data retry.");
    }

    private static async Task TerminalResumeIncompleteWaitsForFinalization()
    {
        using var fixture = new PublishAssetFixture(32);
        var handler = new TerminalResumeIncompleteHandler(
            finalizeOnStatusRequest: 2);
        using var client = new HttpClient(handler)
        {
            Timeout = Timeout.InfiniteTimeSpan,
        };
        ...
        TestAssert.Equal("video-after-terminal-308", id,
            "A terminal resume-incomplete response should recover the finalized video identifier.");
        TestAssert.Equal(1, handler.DataRequests,
            "Terminal finalization polling must not resend the completed data chunk.");
        TestAssert.Equal(2, handler.StatusRequests,
            "Terminal finalization polling should stop as soon as the identifier arrives.");
    }

    private static async Task TerminalResumeIncompleteBoundsFinalization()
    {
        using var fixture = new PublishAssetFixture(32);
        var handler = new TerminalResumeIncompleteHandler(
            finalizeOnStatusRequest: null);
        using var client = new HttpClient(handler)
        {
            Timeout = Timeout.InfiniteTimeSpan,
        };
        ...
        YouTubePublishingException exception =
            await TestAssert.ThrowsAsync<YouTubePublishingException>(
                () => api.UploadVideoAsync(
                    ...
                    null,
                    CancellationToken.None),
                "A terminal resume-incomplete response must use bounded finalization polling.");

        TestAssert.Equal("youtube.upload.finalization-incomplete",
            exception.DiagnosticCode,
            "Terminal 308 finalization failure should retain the stable diagnostic code.");
        TestAssert.Equal(1, handler.DataRequests,
            "A terminal resume-incomplete response must never resend completed bytes.");
        TestAssert.Equal(3, handler.StatusRequests,
            "Terminal finalization polling must retain its fixed attempt bound.");
    }
    ...
    private sealed class FinalizingYouTubeHandler : HttpMessageHandler
    {
        private readonly int? _finalizeOnStatusRequest;

        public FinalizingYouTubeHandler(int? finalizeOnStatusRequest) =>
            _finalizeOnStatusRequest = finalizeOnStatusRequest;

        public int DataRequests { get; private set; }
            ...
            CancellationToken cancellationToken)
        {
            if (request.Method == HttpMethod.Post)
            {
                ...
                session.Headers.Location = new Uri(
                    "https://upload.youtube.googleapis.com/session/finalizing");
                return session;
            }
            ...
            {
                DataRequests++;
                var transient = new HttpResponseMessage(
            ...
            StatusRequests++;
            if (_finalizeOnStatusRequest == StatusRequests)
            {
                return new HttpResponseMessage(HttpStatusCode.OK)
                {
                    Content = new StringContent(
                        "{\"id\":\"video-after-status\"}",
                        Encoding.UTF8,
                        "application/json"),
                };
            }

            var pending = new HttpResponseMessage((HttpStatusCode)308);
            pending.Headers.TryAddWithoutValidation("Range", "bytes=0-31");
            return pending;
        }
    }

    private sealed class TerminalResumeIncompleteHandler : HttpMessageHandler
    {
        private readonly int? _finalizeOnStatusRequest;

        public TerminalResumeIncompleteHandler(int? finalizeOnStatusRequest) =>
            _finalizeOnStatusRequest = finalizeOnStatusRequest;
            ...
            CancellationToken cancellationToken)
        {
            if (request.Method == HttpMethod.Post)
            {
                var session = new HttpResponseMessage(HttpStatusCode.OK);
                session.Headers.Location = new Uri(
                    "https://upload.youtube.googleapis.com/session/terminal-308");
                return session;
            }
    ...
    private sealed class RecordingHistoryDialog :
        IPublishHistoryDialogService
    {
        public int CallCount { get; private set; }

        public void Show(PublishHistoryViewModel viewModel) => CallCount++;
    }

    private sealed class RecordingHistoryLinkLauncher :
    ...
    private static YouTubePublishHistoryEntry CreateHistoryEntry(
        int index,
    ...
    {
        bool didNotFinish = outcome is
            YouTubePublishOutcome.Failed or
            YouTubePublishOutcome.Cancelled;
        string? videoId = didNotFinish ? null : $"video-{index}";
        string? videoUrl = videoId is null
            ? null
            : $"https://www.youtube.com/watch?v={videoId}";
        return new YouTubePublishHistoryEntry(
            $"history-{index}",
            $"asset-{index}",
            $"Moment {index}",
            videoId,
            ...
            outcome == YouTubePublishOutcome.Scheduled
                ? attemptedAtUtc.AddDays(2)
                : null,
            didNotFinish ? "did-not-finish" : null,
            didNotFinish ? "The upload did not finish." : null,
            remoteStatus,
        ...
        IClipEditorialMetadataGenerationService
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var generator = new ClipEditorialMetadataGeneratorIdentity(
