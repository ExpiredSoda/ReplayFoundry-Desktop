...
internal static class GenerateWorkflowStateOwnerTests
{
    ...
    [
        ...
        new(
            "Operation controller stop permits reentrant cancellation completion",
            OperationControllerStopPermitsReentrantCompletion),
        new(
            "Operation controller stop seals future operations",
            OperationControllerStopSealsFutureOperations),
        new(
    ...
    private static async Task OperationControllerStopPermitsReentrantCompletion()
    {
        using var controller = new GenerationOperationController();
        GenerationOperationLease active = controller.Begin(
            GenerationOperationKind.Generation);
        using CancellationTokenRegistration registration =
            active.CancellationToken.Register(active.Dispose);

        Task stop = Task.Run(() =>
            controller.StopAsync(CancellationToken.None));
        Task completed = await Task.WhenAny(
            stop,
            Task.Delay(TimeSpan.FromSeconds(2)));

        TestAssert.True(ReferenceEquals(completed, stop),
            "A synchronous cancellation callback that completes the lease must not deadlock StopAsync.");
        await stop;
        TestAssert.False(controller.HasActiveOperation,
            "Reentrant cancellation completion must clear the active operation.");
    }

    private static async Task OperationControllerStopSealsFutureOperations()
    {
        using var controller = new GenerationOperationController();
        await controller.StopAsync(CancellationToken.None);

        TestAssert.Throws<InvalidOperationException>(
            () => controller.Begin(GenerationOperationKind.Generation),
            "Generate must not reopen work after application quiescence begins.");
    }
