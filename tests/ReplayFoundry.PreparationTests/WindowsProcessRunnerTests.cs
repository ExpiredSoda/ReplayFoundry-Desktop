...
internal static class WindowsProcessRunnerTests
{
    ...
    private const string InheritedSecretEnvironmentVariable =
        "REPLAYFOUNDRY_PROCESS_RUNNER_INHERITED_SECRET";
    ...
    [
        ...
        new(
            "Windows process runner can replace the inherited environment",
            ReplacesInheritedEnvironment),
        new(
    ...
    private static async Task ReplacesInheritedEnvironment()
    {
        string root = CreateTestRoot();
        string? original = Environment.GetEnvironmentVariable(
            InheritedSecretEnvironmentVariable,
        ...
        try
        {
            Environment.SetEnvironmentVariable(
                ...
                EnvironmentVariableTarget.Process);
            var request = CreateSelfRequest(
                [EchoMode],
                root,
                new Dictionary<string, string>
                {
                    [TestEnvironmentVariable] = "explicit-value",
                },
                inheritParentEnvironment: false);
            ProcessRunResult result = await new WindowsProcessRunner().RunAsync(
                request,
                CancellationToken.None);
            EchoPayload? payload = JsonSerializer.Deserialize<EchoPayload>(
                result.StandardOutput);
            TestAssert.True(payload is not null, "The isolated child returned no payload.");
            TestAssert.Equal(
                "explicit-value",
                payload!.EnvironmentValue,
                "The isolated child lost an explicit environment value.");
            TestAssert.True(
                payload.InheritedEnvironmentValue is null,
                "The isolated child inherited a parent-only secret value.");
        }
        finally
        {
            Environment.SetEnvironmentVariable(
                ...
                original,
                EnvironmentVariableTarget.Process);
            await DeleteTestRootAsync(root);
        }
    }
    ...
    private sealed record EchoPayload(
        ...
        string? InheritedEnvironmentValue);
}
