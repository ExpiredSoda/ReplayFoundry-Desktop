...
internal static partial class UiUxApplicationSurfaceTests
{
    ...
    private static Task BugReportOfficialEndpointIsEmbedded()
    {
        const string expected =
            "https://replayfoundry.com/api/v1/user-reports";
        System.Reflection.Assembly assembly =
            typeof(ReplayFoundry.Desktop.App).Assembly;
        string endpoint = assembly
            .GetCustomAttributes(
            ...
            .Single(attribute => string.Equals(
                attribute.Key,
                UserReportTransportFactory.EndpointMetadataKey,
                StringComparison.Ordinal))
            .Value ?? string.Empty;
        TestAssert.Equal(
            expected,
            endpoint,
            "Every ordinary desktop build must embed the fixed official report endpoint.");

        IUserReportTransport transport =
            UserReportTransportFactory.CreateFromAssembly(assembly);
        try
        {
            TestAssert.True(
                transport.IsConfigured,
                "The composed desktop report transport must be available in normal builds.");
            TestAssert.Equal(
                "Replay Foundry support",
                transport.DestinationDisplayName,
                "Settings must identify the reviewed report destination.");
        }
        finally
        {
    ...
    private static async Task BugReportTimeoutRemainsLocal()
    {
        var consent = new UserReportConsentState(
            new InMemoryUserReportConsentStore());
        consent.Enable(DateTimeOffset.UtcNow);
        ...
        var coordinator = new UserReportCoordinator(
            ...
            new ReplayFoundryDiagnosticCollector(),
            new UserReportSanitizer(),
            new TimeoutUserReportTransport());
        StoredUserReport saved = coordinator.SaveManual(
            "Timeout report",
            "The transport timeout must remain retryable.",
            includeDiagnostics: false);

        UserReportSubmissionResult result = await coordinator.SendAsync(
            saved.Draft.ReportId);

        TestAssert.Equal(
            UserReportSubmissionCode.Failed,
            result.Code,
            "An internal transport timeout should be reported as a failed send.");
        StoredUserReport retained = outbox.Current.Single();
        TestAssert.Equal(
            UserReportDisposition.Failed,
            retained.Disposition,
            "The timed-out report should remain in the local outbox.");
        TestAssert.Equal(
            nameof(TaskCanceledException),
            retained.FailureCode,
            "The outbox should retain the typed transport-timeout failure.");
    }
    ...
    private static async Task GameContextRecoveryFollowsPreferenceReset()
    {
        string root = TemporaryRoot("GameContextRecoveryReset");
        try
        {
            string memory = Path.Combine(root, "game-context-memory.json");
            string recovery = Path.Combine(
                root,
                "Diagnostics",
                "GameContextMemory",
                "game-context-memory.json.example.invalid.json");
            string unrelatedDiagnostic = Path.Combine(
                root,
                "Diagnostics",
                "Outbox",
                "report.json");
            Write(memory, "remembered game");
            Write(recovery, "invalid remembered game");
            Write(unrelatedDiagnostic, "saved report");
            var service = new ReplayFoundryLocalDataMaintenanceService(root);

            service.ScheduleReset(new ReplayFoundryLocalDataResetRequest(
                [ReplayFoundryLocalDataKind.PreferencesAndHistory]));
            ReplayFoundryLocalDataCleanupResult result =
                await service.ApplyScheduledResetAsync();
            ...
            TestAssert.True(
                result.Succeeded,
                "The preference reset should delete active and recovered game memory.");
            TestAssert.False(
                File.Exists(memory),
                "The active game-context memory must follow the preference reset.");
            TestAssert.False(
                Directory.Exists(Path.GetDirectoryName(recovery)),
                "Recovered game-context payloads must follow the same reset.");
            TestAssert.True(
                File.Exists(unrelatedDiagnostic),
                "A preference-only reset must preserve unrelated saved reports.");
        }
        finally
        {
    ...
    private sealed class TimeoutUserReportTransport : IUserReportTransport
    {
