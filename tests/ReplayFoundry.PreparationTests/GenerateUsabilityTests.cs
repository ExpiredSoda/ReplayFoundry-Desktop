...
internal static class GenerateUsabilityTests
{
    ...
    [
        ...
        new("Remembered audio roles require a stable normalized stream identity", RememberedAudioRolesRequireStableIdentity),
    ...
    private static Task RememberedAudioRolesRequireStableIdentity()
    {
        PreparedGenerationSource source = CreatePreparedSource(
            "remembered-stream-identity.mkv",
            audioStreamCount: 2);
        MediaProbeResult normalizedTitleVariant = CreateAudioIdentityVariant(
            source.Media,
            streamIndex: 2,
            title: "  misleading   TITLE 2 ",
            channelLayout: "stereo");
        MediaProbeResult changedTitleVariant = CreateAudioIdentityVariant(
            source.Media,
            streamIndex: 2,
            title: "Track3_vertical",
            channelLayout: "stereo");
        MediaProbeResult changedLayoutVariant = CreateAudioIdentityVariant(
            source.Media,
            streamIndex: 2,
            title: "Misleading title 2",
            channelLayout: "2.0");

        string fingerprint = GenerationAudioLayoutFingerprint.Create(
            source.Media);
        TestAssert.Equal(
            fingerprint,
            GenerationAudioLayoutFingerprint.Create(normalizedTitleVariant),
            "Case and spacing differences must not discard a confirmed capture layout.");
        TestAssert.False(
            fingerprint.Equals(
                GenerationAudioLayoutFingerprint.Create(changedTitleVariant),
                StringComparison.Ordinal),
            "A changed stream identity hint must require role confirmation rather than reusing a potentially wrong track.");
        TestAssert.False(
            fingerprint.Equals(
                GenerationAudioLayoutFingerprint.Create(changedLayoutVariant),
                StringComparison.Ordinal),
            "A changed reported channel layout must require role confirmation.");

        string root = Path.Combine(
            Path.GetTempPath(),
            "ReplayFoundryAudioRoleMemoryTests",
            Guid.NewGuid().ToString("N"));
        string path = Path.Combine(root, "roles.json");
        ...
        try
        {
                  ...
                  "Entries": [
                    {
                      ...
                      "AbsoluteAudioStreamIndex": 2,
                      "ContentRole": "CreatorCommentary",
                      "LanguagePolicy": "English",
                      "ConfirmedAtUtc": "2026-08-25T02:00:50Z"
                    }
                  ]
                }
                """);
            var memory = new JsonGenerationAudioRoleMemory(path);
            TestAssert.Null(
                memory.Find(source),
                "Legacy structural-only role entries must be ignored so ambiguous tracks are reconfirmed.");
            ...
            memory.Remember([source], [selection]);
            RememberedGenerationAudioRole? remembered = memory.Find(source);
            TestAssert.True(
                remembered is not null &&
                remembered.AbsoluteAudioStreamIndex == 2 &&
                remembered.ContentRole == CaptionAudioContentRole.MixedSpeech,
                "A newly confirmed identity-aware layout should be remembered exactly.");

            var changedSource = new PreparedGenerationSource(
                source.Source,
                changedTitleVariant,
                source.FileSnapshot);
            TestAssert.Null(
                memory.Find(changedSource),
                "A title-identity change must never select or classify a stream automatically.");
            TestAssert.True(
                File.ReadAllText(path).Contains(
                    "\"SchemaVersion\": \"2.0\"",
                    StringComparison.Ordinal),
                "A fresh confirmation must replace the legacy memory document with the identity-aware schema.");
        }
        finally
        {
    ...
    private static async Task RecentProjectsReopenStudioDrafts()
    {
        string root = CreateRoot();
        ...
        StudioProjectSwitchResult firstSwitch =
            await studio.TrySwitchProjectAsync(retainedFirst!);
        ...
        StudioProjectSwitchResult finalizedSwitch =
            await studio.TrySwitchProjectAsync(retainedFinalized!);
    ...
    private static Task ClipCountQualityRelationshipIsExplicit()
    {
        ...
        TestAssert.Equal("Preferred quality", viewModel.QualityControlLabel,
            "Fill requested count should describe the preferred starting quality in plain language.");
        TestAssert.True(
            viewModel.CountQualityRelationshipDescription.Contains(
                "best safe alternatives",
                StringComparison.OrdinalIgnoreCase),
            "Fill-count behavior must explain how Replay Foundry reaches the requested amount.");

        viewModel.SelectedFulfillmentOption = viewModel.FulfillmentOptions
            .Single(option =>
                option.Value == ClipFulfillmentPreference.QualityFirst);
        TestAssert.Equal("Minimum quality", viewModel.QualityControlLabel,
            "Quality-first should describe the minimum accepted quality.");
        TestAssert.True(
            viewModel.CountQualityRelationshipDescription.Contains(
                "fewer clips",
                StringComparison.OrdinalIgnoreCase),
    ...
    private static MediaProbeResult CreateAudioIdentityVariant(
        MediaProbeResult media,
        int streamIndex,
        string title,
        string channelLayout)
    {
        AudioStreamInfo[] audioStreams = media.AudioStreams
            .Select(stream => stream.Index != streamIndex
                ? stream
                : new AudioStreamInfo(
                    stream.Index,
                    stream.CodecName,
