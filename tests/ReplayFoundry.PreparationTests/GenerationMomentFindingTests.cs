...
internal static class GenerationMomentFindingTests
{
    ...
    [
        ...
        new("Hidden Moments default to AI-required metadata", HiddenMomentsDefaultToAiRequired),
    ...
    private static Task HiddenMomentsDefaultToAiRequired()
    {
        object? defaultValue = typeof(GenerationHiddenMoment)
            .GetConstructors()
            .Single()
            .GetParameters()
            .Single(parameter => parameter.Name == "editorialPreference")
            .DefaultValue;

        TestAssert.Equal(
            ClipEditorialGenerationPreference.AiRequired,
            (ClipEditorialGenerationPreference)defaultValue!,
            "Omitting a hidden-moment metadata preference must require AI instead of silently opting into heuristics.");
    ...
    private static Task HiddenMomentsCarryEditorialTier()
    {
        ...
        GenerationMomentFindingRequest heuristicRequest = CreateRequest(
            sourceCount: 1,
            desiredCount: 1,
            analysisDepth: GenerationAnalysisDepth.Balanced,
        ...
        GenerationHiddenMoment thorough = GenerationHiddenMomentPlanner.Create(
            ...
            .Moments[0];
        GenerationHiddenMoment heuristic =
            GenerationHiddenMomentPlanner.Create(
                new GenerationMomentFindingService(finder).Find(
                    heuristicRequest))
                .Moments[0];
        ...
        TestAssert.Equal(
            ...
            "Balanced scan depth must not silently opt out of AI metadata.");
        TestAssert.Equal(
            ClipEditorialGenerationPreference.AiRequired,
            thorough.EditorialPreference,
            "Thorough Hidden Moments must require qualified AI.");
        TestAssert.Equal(
            ClipEditorialGenerationPreference.HeuristicOnly,
            heuristic.EditorialPreference,
            "Only the explicit no-AI authoring choice may retain heuristics.");
        return Task.CompletedTask;
    }
