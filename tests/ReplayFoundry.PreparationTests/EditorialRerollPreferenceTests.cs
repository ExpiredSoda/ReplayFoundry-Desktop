...
internal static class EditorialRerollPreferenceTests
{
    ...
    private static Task ProviderChoicePersists()
    {
        ...
        TestAssert.True(
            initial.UseLocalAi,
            "A missing preference must default to required local AI; heuristics require an explicit opt-out.");

        File.WriteAllText(
            path,
            "{\"schemaVersion\":\"replayfoundry-editorial-reroll-preference-1.0\"}");
        var omittedField = new EditorialRerollPreferenceState(
            new JsonEditorialRerollPreferenceStore(path));
        TestAssert.True(
            omittedField.UseLocalAi,
            "An omitted provider field must also fail safe to required local AI.");

        initial.SetUseLocalAi(false);
        var reloaded = new EditorialRerollPreferenceState(
            new JsonEditorialRerollPreferenceStore(path));

        TestAssert.False(
            reloaded.UseLocalAi,
            "The explicit no-AI choice must survive a new store instance.");

        reloaded.SetUseLocalAi(true);
        var enabledAgain = new EditorialRerollPreferenceState(
            new JsonEditorialRerollPreferenceStore(path));
        TestAssert.True(
            enabledAgain.UseLocalAi,
            "Turning local AI back on must persist the required-AI provider choice.");
