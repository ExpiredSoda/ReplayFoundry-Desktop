...
class GroundedMetadataAdaptiveSamplingTests(unittest.TestCase):
    ...
    def test_three_minute_editorial_review_uses_four_bounded_windows(self) -> None:
        self.assertEqual(180, GROUNDED_MAXIMUM_REVIEW_DURATION_SECONDS)
        plan = adaptive_sampling_plan(
            float(GROUNDED_MAXIMUM_REVIEW_DURATION_SECONDS)
        )

        self.assertEqual(4, len(plan))
        self.assertEqual(
            [CONTEXT_TIER, CORE_TIER, CORE_TIER, CONTEXT_TIER],
            [window.tier for window in plan],
        )
        self.assertTrue(all(window.maximum_frames <= 6 for window in plan))
        self.assertTrue(all(
            window.maximum_total_video_pixels <= VIDEO_TOTAL_PIXEL_BUDGET
            for window in plan
        ))
