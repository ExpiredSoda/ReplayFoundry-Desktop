...
class GroundedMetadataPacketReuseTests(unittest.TestCase):
            ...
            return (
                {
                    ...
                    "editorialFrame": {
                        ...
                        "momentKind": "Action",
                        "premise": "A visible object changed",
                        "supportingDraftOrdinals": [2],
    ...
    def test_one_missing_primary_event_preserves_other_batch_results(self) -> None:
        requests = [
            _request(0, "DirectAction"),
            _request(1, "SpecificCuriosity"),
            _request(2, "OutcomeFocused"),
        ]
        requests[1]["evidence"][0]["description"] = "Different evidence."
        synthesis_calls = []

        def build(request, *_args):
            if request["attempt"] == 1:
                raise NoDistinctPrimaryVisualEventError(
                    "Sensitive implementation detail must not cross the wire."
                )
            return _new_grounding_packet(
                request,
                1,
        ...
        def synthesize(request, _ordinal, _prompt, _packet, _reused, *_args):
            prior_titles = _args[-1]
            synthesis_calls.append((request["attempt"], len(prior_titles)))
            return {
                "candidateId": request["candidateId"],
                "attempt": request["attempt"],
                "metadata": {
                    "title":
                        f"Opened supported angle {request['attempt']} #ExampleGame"
                },
            }
        ...
        self.assertEqual([0, 2], [item["attempt"] for item in (results[0], results[2])])
        self.assertEqual(
            {
                "candidateId": "candidate-shared",
                "attempt": 1,
        ...
        self.assertNotIn("Sensitive", json.dumps(results[1]))
        self.assertEqual([(0, 0), (2, 1)], synthesis_calls)

    def test_other_inference_failures_remain_whole_host_failures(self) -> None:
        with (
            patch.object(grounded_metadata_command, "_set_failure_case"),
            patch.object(grounded_metadata_command, "_set_failure_stage"),
            patch.object(
                grounded_metadata_command,
                "_build_grounding_packet",
                side_effect=InferenceError("Runtime inference failed."),
            ),
        ):
            with self.assertRaisesRegex(InferenceError, "Runtime inference failed"):
                grounded_metadata_command._infer_grouped_requests(
                    [_request(0, "DirectAction")],
                    ["1" * 64],
                    "prompt",
        ...
        packet = _new_grounding_packet(
            ...
            0.5,
            {
                ...
                "editorialFrame": {
                    ...
                    "authorityKind": "StoryShapeOnly",
                    "momentKind": "Action",
                    "premise": "A gate opened along the route",
