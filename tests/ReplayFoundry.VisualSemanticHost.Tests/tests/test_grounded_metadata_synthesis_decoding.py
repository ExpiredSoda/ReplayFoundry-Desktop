...
from _test_bootstrap import HOST_ROOT
...
from replayfoundry_visual_semantic.editorial.grounded_metadata_recovery_candidate_selection import (
...
class GroundedMetadataSynthesisDecodingTests(unittest.TestCase):
    ...
    def test_policy_text_hash_and_constants_are_frozen(self) -> None:
        policy_path = HOST_ROOT / POLICY_FILE_NAME
        normalized = (
    ...
    def test_case_local_fact_retention_remains_auditable_review_provenance(
        self,
    ) -> None:
        issues = complete_review_issues(
            {
                "title": "Reached the visible doorway",
                "description": "The open doorway led into the next hall.",
            },
            ["CaseLocalFactRetention"],
        )

        self.assertEqual(("CaseLocalFactRetention",), issues)
