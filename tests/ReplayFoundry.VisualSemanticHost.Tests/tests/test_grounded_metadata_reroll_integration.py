            ...
            "visualEventSelectionAssessments": [
                {
                    ...
                    "presentationKind": "InteractiveGameplay",
                }
            ],
            "editorialFrame": {
                ...
                "momentKind": "Action",
                "premise": "A supported gate interaction changed the route",
...
def _reviewable_generated(
    ...
    hashtag = "#ExampleGame"
    owns_hashtag = title.endswith(" " + hashtag)
    title_body = title[: -(len(hashtag) + 1)] if owns_hashtag else title
    normalized_title_body = normalize_terminal_single_period_title_body(title_body)
    normalized_title = (
        normalized_title_body + " " + hashtag
        if owns_hashtag
    ...
    return (
        {
            ...
            "tags": ["door"],
            "grounding": [],
            "_reviewIssues": list(review_issues),
        },
        SimpleNamespace(
            generated_token_count=20,
            maximum_new_tokens=768,
...
class GroundedMetadataRerollIntegrationTests(unittest.TestCase):
    ...
    def test_input_contract_preserves_only_ordered_clip_bounded_transcript_spans(self):
        request = _request(1, "DirectAction")
        validated_video = request.pop("_validated")
        request["reviewVideo"] = {"placeholder": True}
        request["transcripts"] = [
            {
                "absoluteAudioStreamIndex": 1,
                "role": "CreatorSpeech",
                "authority": "UserCorrected",
                "text": "I found the route. That changed everything.",
                "spans": [
                    {
                        "startSeconds": 11.0,
                        "endSeconds": 12.0,
        ...
        self.assertEqual(
            request["transcripts"][0]["spans"],
            validated["transcripts"][0]["spans"],
        )

        request["transcripts"][0]["spans"][1]["startSeconds"] = 11.5
        with patch.object(
            grounded_metadata_contract,
            "_validate_review_video",
            return_value=validated_video,
    ...
    def test_pool_prefers_grounded_repetition_over_masked_interpretation(self):
        source = _request(0, "ConcreteDetail")
        packet = _packet(source)
        outputs = [
            _rejected_error(
                ...
                "title opening.",
                "A person opens the visible door",
            ),
            _rejected_error(
            ...
            _rejected_error(
                ...
                "I moved another person's body through the door",
            ),
            _reviewable_generated(
                "Man sits at desk in dim room #ExampleGame",
                "A man sits at a desk in a dim room, which suggests a moment "
                "of quiet focus or reflection.",
                # The strict parser surfaced tense first. Recovery selection
                # must independently discover the masked interpretation.
                ["NonRetrospectiveVoice"],
                "1" * 64,
            ),
            _reviewable_generated(
                "I opened menu, moved through door, walked down hallway with "
                "red light #ExampleGame",
                "I opened the menu, moved through the door, and walked down "
                "the hallway with red light.",
                ["TitleDescriptionRepetition"],
                "2" * 64,
            ),
            _reviewable_generated(
                "I guided another person through the doorway #ExampleGame",
                "I controlled another person's movement through the doorway.",
                ["UnsupportedCreatorEmbodiment"],
                "3" * 64,
            ),
            _reviewable_generated(
                "A person waited beside the dim doorway #ExampleGame",
                "The still figure suggested a tense moment before the doorway.",
                ["NonRetrospectiveVoice"],
                "4" * 64,
            ),
        ]

        result = _synthesize(source, packet, (), outputs)

        generation = result["generation"]
        self.assertEqual(4, generation["synthesisRecoveryPoolAttemptedCandidateCount"])
        self.assertEqual(2, generation["synthesisRecoveryPoolSelectedCandidateOrdinal"])
        self.assertEqual(
            "I opened menu, moved through door, walked down hallway with red "
            "light #ExampleGame",
            result["metadata"]["title"],
        )
        self.assertEqual(
            ["TitleDescriptionRepetition"],
            generation["metadataReviewIssues"],
        )
        self.assertEqual("2" * 64, generation["decodedTextSha256"])
        pool_attestations = generation["synthesisPassAttestations"][3:]
        self.assertEqual([3407, 3408, 3409, 3410], [
            value["seed"] for value in pool_attestations
        ])
        self.assertEqual([False, True, False, False], [
            value["accepted"] for value in pool_attestations
        ])
        self.assertEqual(
            [
            ...
            [value["rejectionCode"] for value in pool_attestations],
        )
        self.assertEqual("2" * 64, pool_attestations[1]["outputSha256"])

    def test_reviewable_pool_candidate_does_not_stop_strict_search(self):
        source = _request(0, "ConcreteDetail")
        packet = _packet(source)
        outputs = [
            _rejected_error(
            ...
            _reviewable_generated(
                "Man sits at desk in dim room #ExampleGame",
                "A man sits in a dim room, which suggests quiet reflection.",
                ["NonRetrospectiveVoice"],
                "1" * 64,
            ),
            _generated("Opened the visible door behind us #ExampleGame"),
        ]
        ...
        self.assertEqual(2, generation["synthesisRecoveryPoolSelectedCandidateOrdinal"])
        self.assertFalse(generation["metadataReviewRequired"])
        self.assertEqual([], generation["metadataReviewIssues"])
        self.assertEqual(
            ["UnsupportedMentalState", None],
            [
                value["rejectionCode"]
                for value in generation["synthesisPassAttestations"][3:]
            ],
        )

    def test_pool_prefers_complete_review_copy_over_structural_findings(self):
        source = _request(0, "ConcreteDetail")
        packet = _packet(source)
        outputs = [
            _rejected_error(
            ...
            _reviewable_generated(
                "Opened the #ExampleGame",
                "The visible doorway remained open beside the hall.",
                ["IncompleteTitle"],
                "1" * 64,
            ),
            _reviewable_generated(
                "Opened the visible doorway #ExampleGame",
                "The visible doorway remained open beside the hall.",
                ["EmbeddedHashtag"],
                "2" * 64,
            ),
            _reviewable_generated(
                "Opened the visible doorway #ExampleGame",
                "Opened the visible doorway beside the hall.",
                ["TitleDescriptionRepetition"],
                "3" * 64,
            ),
            _reviewable_generated(
                "Waited beside the visible doorway #ExampleGame",
                "The figure waited for the doorway to open.",
                ["UnsupportedMentalState"],
                "4" * 64,
            ),
        ]
        ...
        generation = result["generation"]
        self.assertEqual(4, generation["synthesisRecoveryPoolAttemptedCandidateCount"])
        self.assertEqual(3, generation["synthesisRecoveryPoolSelectedCandidateOrdinal"])
        self.assertEqual(
            "Opened the visible doorway #ExampleGame",
            result["metadata"]["title"],
        )
        ...
        self.assertEqual(
            ["IncompleteTitle", "GameHashtag", None, "UnsupportedMentalState"],
            [
                value["rejectionCode"]
                for value in generation["synthesisPassAttestations"][3:]
            ],
    ...
    def test_terminal_only_pool_still_returns_least_bad_complete_ai_copy(self):
        source = _request(0, "ConcreteDetail")
        packet = _packet(source)
        outputs = [
            _rejected_error(
            ...
            _reviewable_generated(
                "Opened the #ExampleGame",
                "The doorway remained open beside the hall.",
                ["IncompleteTitle"],
                "1" * 64,
            ),
            _reviewable_generated(
            ...
            _reviewable_generated(
                "Guided the figure through the doorway #ExampleGame",
                "I controlled another person's movement through the doorway.",
                ["UnsupportedCreatorEmbodiment"],
                "3" * 64,
            ),
            _reviewable_generated(
                "Named the unseen chamber #ExampleGame",
                "The copy relied on an unresolved visual claim.",
    ...
    def test_review_only_pool_returns_best_complete_ai_copy_for_review(self):
        source = _request(0, "ConcreteDetail")
        packet = _packet(source)
        outputs = [
            _rejected_error(
            ...
            _reviewable_generated(
                "A person opened the doorway #ExampleGame",
                "A person crossed the threshold into the hall.",
            ...
            _reviewable_generated(
                "The doorway opened into the hall #ExampleGame",
                "The scene showed the doorway beside the hall.",
                ["GenericOpening"],
                "3" * 64,
            ),
            _reviewable_generated(
                "Found the visible doorway open #ExampleGame",
                "The open doorway led toward the hall.",
                ["RerollTitleTooSimilar"],
                "4" * 64,
            ),
        ]
    ...
    def test_factual_authority_rejection_disables_sticky_target(self):
        ...
        attestations = result["generation"]["synthesisPassAttestations"]
        self.assertTrue(attestations[0]["retryAnchorCaptured"])
        self.assertFalse(attestations[0]["retryAnchorApplied"])
        self.assertIsNone(attestations[0]["retryAnchorDisabledReason"])
        self.assertTrue(attestations[1]["retryAnchorApplied"])
        self.assertIsNone(
            attestations[1]["retryAnchorDisabledReason"],
            "A pass attests the anchor state it actually used, before its output "
            "can invalidate that state.",
        )
        self.assertFalse(attestations[2]["retryAnchorApplied"])
        self.assertEqual(
            "UnsupportedCreatorEmbodiment",
