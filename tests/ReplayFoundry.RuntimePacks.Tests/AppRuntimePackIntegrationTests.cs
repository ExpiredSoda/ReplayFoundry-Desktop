...
internal static class AppRuntimePackIntegrationTests
{
    ...
    private static Task AdvancedRemovalRequestsGracefulClose()
    {
        var withWindow = new RecordingApplicationLifecycle(hasMainWindow: true);
        RuntimeMaintenanceShutdownCoordinator.Request(withWindow);
        Assert(
            withWindow.CloseRequests == 1 && withWindow.ForceShutdowns == 0,
            "Advanced removal bypassed the cancelable main-window close path.");

        var withoutWindow = new RecordingApplicationLifecycle(hasMainWindow: false);
        RuntimeMaintenanceShutdownCoordinator.Request(withoutWindow);
        Assert(
            withoutWindow.CloseRequests == 1 && withoutWindow.ForceShutdowns == 1,
            "Advanced removal did not reserve forced shutdown for the no-window fallback.");
        return Task.CompletedTask;
    }
    ...
    private sealed class RecordingApplicationLifecycle(bool hasMainWindow) :
        IRuntimeMaintenanceApplicationLifecycle
    {
        public int CloseRequests { get; private set; }
        public int ForceShutdowns { get; private set; }
        ...
        public void ForceShutdown() => ForceShutdowns++;
    }
