...
internal static class RuntimePackTests
{
    ...
    [
        ...
        new("Active-pack startup snapshot isolates one corrupt capability", ActiveSnapshotIsolatesCorruptPack),
        ...
        new("Cleanup restores a selected pack stranded in removal quarantine", SelectedRemovalQuarantineRestores),
        new("Cleanup deletes only proven inactive removal quarantines", InactiveRemovalQuarantineCleansSafely),
        new("Repair and cleanup acquire the store gate before touching staging", RepairAndCleanupShareStoreGate),
        new("Independent stores serialize mutations through one process lease", IndependentStoresShareMutationLease),
        ...
        new("Failed active-selection removal rolls the pack back", FailedRemovalRestoresPack),
    ...
    private static async Task ActiveSnapshotIsolatesCorruptPack()
    {
        using Fixture fixture = new();
        using ReplayFoundryRuntimePackStore store = fixture.Store();
        ...
        ReplayFoundryRuntimePackManifest speech = CreateSpeechManifest(
            fixture,
            [new("replayfoundry-media-tools", "1.0.0", media.ManifestHash)]);
        await fixture.SealAsync(speech);
        InstalledReplayFoundryRuntimePack installedSpeech = await store.InstallAsync(
            fixture.Source,
            activate: true);
        await File.AppendAllTextAsync(
            installedSpeech.Resolve(
                ReplayFoundryRuntimeFileRole.SpeechActivityModel),
            "corrupt");

        IReadOnlyList<ReplayFoundryRuntimePackResolution> snapshot =
            await store.ResolveActiveSnapshotAsync(
                [
        ...
        Assert(
            snapshot.Single(item => item.Kind == ReplayFoundryRuntimePackKind.MediaTools).Pack is not null,
            "A corrupt optional capability hid the valid active media pack.");
        ReplayFoundryRuntimePackResolution failedSpeech = snapshot.Single(
            item => item.Kind == ReplayFoundryRuntimePackKind.SpeechActivity);
        Assert(
            failedSpeech.Pack is null && failedSpeech.Failure is InvalidDataException,
            "The startup snapshot did not retain the isolated corrupt-pack failure.");
    }
    ...
    private static async Task SelectedRemovalQuarantineRestores()
    {
        using Fixture fixture = new();
        ReplayFoundryRuntimePackManifest manifest = CreateManifest(fixture);
        await fixture.SealAsync(manifest);
        using ReplayFoundryRuntimePackStore store = fixture.Store();
        InstalledReplayFoundryRuntimePack installed = await store.InstallAsync(
            fixture.Source,
            activate: true);
        string quarantine = installed.RootDirectory + ".removing-" +
            Guid.NewGuid().ToString("N");
        Directory.Move(installed.RootDirectory, quarantine);

        Assert(
            (await store.ListInstalledAsync()).Count == 0,
            "A removal quarantine leaked into the installed-pack inventory.");
        await store.CleanupAbandonedStagingAsync();

        Assert(
            Directory.Exists(installed.RootDirectory) && !Directory.Exists(quarantine),
            "Cleanup did not restore the selected pack to its canonical directory.");
        InstalledReplayFoundryRuntimePack active = await store.ResolveActiveAsync(
            ReplayFoundryRuntimePackKind.MediaTools);
        Assert(
            active.Manifest.ManifestHash == manifest.ManifestHash,
            "The restored quarantine did not resolve as the selected active pack.");
    }

    private static async Task InactiveRemovalQuarantineCleansSafely()
    {
        using Fixture fixture = new();
        ReplayFoundryRuntimePackManifest inactive = CreateManifest(
            fixture,
            version: "1.0.0");
        await fixture.SealAsync(inactive);
        using ReplayFoundryRuntimePackStore store = fixture.Store();
        InstalledReplayFoundryRuntimePack installedInactive = await store.InstallAsync(
            fixture.Source,
            activate: true);

        File.Delete(Path.Combine(fixture.Source, RuntimePackManifestJson.FileName));
        ReplayFoundryRuntimePackManifest active = CreateManifest(
            fixture,
            version: "2.0.0");
        await fixture.SealAsync(active);
        await store.InstallAsync(fixture.Source, activate: true);

        string inactiveQuarantine = installedInactive.RootDirectory + ".removing-" +
            Guid.NewGuid().ToString("N");
        Directory.Move(installedInactive.RootDirectory, inactiveQuarantine);
        string unknownQuarantine = Path.Combine(
            fixture.StoreRoot,
            "unknown.removing-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(unknownQuarantine);

        await store.CleanupAbandonedStagingAsync();

        Assert(
            !Directory.Exists(inactiveQuarantine),
            "Cleanup retained a manifest-proven inactive quarantine.");
        Assert(
            Directory.Exists(unknownQuarantine),
            "Cleanup deleted an unproven directory that only resembled a quarantine.");
    }

    private static async Task RepairAndCleanupShareStoreGate()
    {
        using Fixture fixture = new();
        await fixture.SealAsync(CreateManifest(fixture));
        using ReplayFoundryRuntimePackStore store = fixture.Store();
        var gate = (SemaphoreSlim?)typeof(ReplayFoundryRuntimePackStore)
            .GetField(
                "_gate",
                System.Reflection.BindingFlags.Instance |
                System.Reflection.BindingFlags.NonPublic)?
            .GetValue(store) ??
            throw new InvalidOperationException("The store operation gate was unavailable.");
        await gate.WaitAsync();
        Task<InstalledReplayFoundryRuntimePack> repair = store.RepairAsync(
            fixture.Source);
        Task cleanup = store.CleanupAbandonedStagingAsync();
        try
        {
            await Task.Delay(TimeSpan.FromMilliseconds(100));
            Assert(
                !repair.IsCompleted && !cleanup.IsCompleted,
                "A staging mutation bypassed the store operation gate.");
            Assert(
                !Directory.Exists(Path.Combine(fixture.StoreRoot, ".staging")),
                "Repair created its workspace before acquiring the store operation gate.");
        }
        finally
        {
            gate.Release();
        ...
        await Task.WhenAll(repair, cleanup);
    }

    private static async Task IndependentStoresShareMutationLease()
    {
        using Fixture fixture = new();
        ...
        using var blocker = new FileStream(
            ...
            FileShare.None);
        using ReplayFoundryRuntimePackStore first = fixture.Store();
        using ReplayFoundryRuntimePackStore second = fixture.Store();

        Task firstMutation = first.CleanupAbandonedStagingAsync();
        Task secondMutation = second.CleanupEmptyStoreAsync();
        await Task.Delay(TimeSpan.FromMilliseconds(150));
        Assert(
            !firstMutation.IsCompleted && !secondMutation.IsCompleted,
            "A store mutation bypassed the shared process lease.");
    ...
    private static async Task FailedRemovalRestoresPack()
    {
        using Fixture fixture = new();
        var paths = new ReplayFoundryRuntimePackStorePaths(fixture.StoreRoot);
        using ReplayFoundryRuntimePackStore store = fixture.Store();
        ReplayFoundryRuntimePackManifest manifest = CreateManifest(fixture);
        await fixture.SealAsync(manifest);
        InstalledReplayFoundryRuntimePack installed =
            await store.InstallAsync(fixture.Source, activate: true);
        using (var selectionLock = new FileStream(
                   paths.ActiveSelectionPath,
                   FileMode.Open,
                   FileAccess.Read,
                   FileShare.Read))
        {
            await AssertThrowsAsync<UnauthorizedAccessException>(
                () => store.RemoveAsync(manifest.Identity.PackageId));
        }
        Assert(
            Directory.Exists(installed.RootDirectory),
            "A failed selection update did not restore the canonical pack directory.");
        InstalledReplayFoundryRuntimePack active = await store.ResolveActiveAsync(
            ReplayFoundryRuntimePackKind.MediaTools);
        Assert(
            active.Manifest.ManifestHash == manifest.ManifestHash,
            "A failed removal changed the active runtime-pack selection.");
        Assert(
            !Directory.EnumerateDirectories(fixture.StoreRoot, "*.removing-*").Any(),
            "A rolled-back removal left an abandoned quarantine.");
    }
