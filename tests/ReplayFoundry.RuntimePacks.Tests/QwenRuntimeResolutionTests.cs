...
internal static class QwenRuntimeResolutionTests
{
    ...
    [
        ...
        new(
            "Qwen qualification reuses the verified Python manifest identity",
            QualificationReusesVerifiedPythonHash),
        new(
            "Qwen resolves the portable packaged model directory against the pack root",
            PortablePackagedModelDirectoryResolves),
        new(
            "Qwen preserves absolute development and verified model directories",
            AbsoluteAndOverrideModelDirectoriesResolve),
        new(
            "Qwen rejects noncanonical relative model directories",
            UnsafeRelativeModelDirectoriesFailClosed),
#if DEBUG
        new(
    ...
    private static async Task QualificationReusesVerifiedPythonHash()
    {
        using var fixture = new Fixture();
        _ = await fixture.InstallAndResolveVerifiedAsync();
        using var store = new ReplayFoundryRuntimePackStore(fixture.Paths);
        ...
        string python = runtime.Resolve(
            ReplayFoundryRuntimeFileRole.PythonExecutable);
        using var exclusivePythonHandle = new FileStream(
            python,
            FileMode.Open,
            FileAccess.Read,
            FileShare.None);
        ...
        Assert(
            ReplayFoundryRuntimeEnvironment.HasMatchingQwenQualification(
                runtime,
                model),
            "Startup reopened Python after its manifest hash was verified.");
    }

    private static Task PortablePackagedModelDirectoryResolves()
    {
        using var fixture = new Fixture();
        ...
        string resolved = Qwen3VlQualifiedEditorialRuntimeLoader
            .ResolveModelDirectoryPath(
                manifestPath,
                "model",
                modelDirectoryOverride: null);
        ...
        Assert(
            ...
            "The portable model path did not resolve to the model directory beside the config directory.");
        return Task.CompletedTask;
    }

    private static Task AbsoluteAndOverrideModelDirectoriesResolve()
    {
        using var fixture = new Fixture();
        string developmentModel = Path.Combine(
            fixture.Root,
            "development-model");
        string verifiedModel = Path.Combine(
            fixture.Root,
            "verified-model");
        Directory.CreateDirectory(developmentModel);
        Directory.CreateDirectory(verifiedModel);

        string absolute = Qwen3VlQualifiedEditorialRuntimeLoader
            .ResolveModelDirectoryPath(
                modelManifestPath: "unused-for-absolute-path",
                developmentModel,
                modelDirectoryOverride: null);
        string overridden = Qwen3VlQualifiedEditorialRuntimeLoader
            .ResolveModelDirectoryPath(
                modelManifestPath: "unused-for-override",
                configuredModelDirectoryPath: "../must-not-be-used",
                modelDirectoryOverride: verifiedModel);

        Assert(
            string.Equals(
                absolute,
                Path.GetFullPath(developmentModel),
                StringComparison.OrdinalIgnoreCase) &&
            string.Equals(
                overridden,
                Path.GetFullPath(verifiedModel),
                StringComparison.OrdinalIgnoreCase),
            "Qwen discarded an absolute development path or verified-pack model override.");
        return Task.CompletedTask;
    }

    private static Task UnsafeRelativeModelDirectoriesFailClosed()
    {
        using var fixture = new Fixture();
        string configDirectory = Path.Combine(
            fixture.Root,
        ...
        foreach (string configuredPath in new[]
                 {
                     "../model",
                     @"..\model",
                     "model/../outside",
                 ...
                 })
        {
            ...
            try
            {
                _ = Qwen3VlQualifiedEditorialRuntimeLoader
                    .ResolveModelDirectoryPath(
                        manifestPath,
                        configuredPath,
                        modelDirectoryOverride: null);
            ...
            Assert(
                rejected,
                $"Qwen accepted noncanonical relative model directory '{configuredPath}'.");
        }
        ...
        string expectedStatus)
    {
        ...
        LocalVisualReviewServices visualServices =
            LocalIntelligenceComposition.CreateVisualReviewServices(
                environment,
                new LocalSpeechServices(
                    CaptionPreparation: null,
                    SpeechActivity: null));
        Assert(
            !visualServices.RuntimeCapabilities.IsEditorialAiAvailable &&
            visualServices.RuntimeCapabilities.EditorialAiUnavailableReason?
                .Contains(
                    expectedStatus,
                    StringComparison.Ordinal) == true,
            "Generation setup discarded the precise Advanced AI incompatibility reason.");
