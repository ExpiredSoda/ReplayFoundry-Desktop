...
public partial class SettingsView : UserControl
{
    private static readonly DependencyPropertyKey IsCompactLayoutPropertyKey =
        DependencyProperty.RegisterReadOnly(nameof(IsCompactLayout), typeof(bool), typeof(SettingsView), new FrameworkPropertyMetadata(false));
    private static readonly DependencyPropertyKey IsStandardLayoutPropertyKey =
        DependencyProperty.RegisterReadOnly(nameof(IsStandardLayout), typeof(bool), typeof(SettingsView), new FrameworkPropertyMetadata(true));
    private static readonly DependencyPropertyKey IsWideLayoutPropertyKey =
        DependencyProperty.RegisterReadOnly(nameof(IsWideLayout), typeof(bool), typeof(SettingsView), new FrameworkPropertyMetadata(false));
        ...
        SelectionChangedEventArgs e)
    {
        if (DataContext is SettingsViewModel viewModel &&
            e.AddedItems.OfType<SettingsSectionItem>().FirstOrDefault() is
            { } selected)
        {
            viewModel.SelectedSection = selected.Key;
        }

        StandardSectionScrollViewport.ScrollToTop();
        CompactSectionScrollViewport.ScrollToTop();
    }
