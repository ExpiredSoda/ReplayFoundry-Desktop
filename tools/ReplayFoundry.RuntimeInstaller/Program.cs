...
internal static class RuntimeInstallerApplication
{
    ...
    public static async Task<int> RunAsync(IReadOnlyList<string> arguments)
    {
        ...
        try
        {
            ...
            bool removeAdvanced = arguments[0].Equals(
                "remove-advanced",
                StringComparison.OrdinalIgnoreCase);
            if (options.ContainsKey("--wait-for-parent") && !removeAdvanced)
                return Usage("--wait-for-parent is valid only with remove-advanced.");
            await WaitForParentExitAsync(options, cancellation.Token);
    ...
    private static async Task WaitForParentExitAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (!options.TryGetValue("--wait-for-parent", out string? value)) return;
        if (!int.TryParse(value, out int processId) || processId <= 0 ||
            processId == Environment.ProcessId)
            throw new ArgumentException("--wait-for-parent requires another positive process ID.");
        try
        {
        ...
        {
            // The parent completed before the helper acquired its process handle.
        }
    }
    ...
    private static int Usage(string error)
    {
        ...
        Console.WriteLine("ReplayFoundry.RuntimeInstaller remove-advanced [--store-root <dir>] [--wait-for-parent <pid>]");
